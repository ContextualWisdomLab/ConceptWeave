...
- Inspect baseline Mermaid output in GitHub, not only its source. If right-side viewer controls cover a terminal node, change the existing graph to `flowchart TB`, reload the exact revision, and confirm every node, edge label, and explanatory paragraph remains visible.
