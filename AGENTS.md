...
- An ontology-library survey is candidate evidence, not adoption: reconcile upstream source and package licences at an immutable coordinate, then require a named RDF/OWL/SKOS/SHACL conformance fixture and fail-closed unsupported-feature result behind a ConceptWeave port/ACL. Do not create a shared utility repository until two independently released consumers prove the same versioned adapter boundary.
