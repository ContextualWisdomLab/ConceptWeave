...
- After moving catalog OIDC policy into versioned configuration, buyer-facing identity guidance must not name retired request-path settings. Keep a regression that forbids the retired key in the returned next action; the owner release remains required before consumer adoption.
