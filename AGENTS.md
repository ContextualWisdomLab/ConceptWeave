...
- Validate public report pending keys for nonblank content before map construction, and return `InvalidInput` for output paths whose `file_name()` is absent; constructor and filesystem boundary tests must cover both cases.
