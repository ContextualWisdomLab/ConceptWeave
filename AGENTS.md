...
- GitHub's Mermaid viewer overlays controls on the right edge. A wide left-to-right dependency graph can render without an error while hiding its terminal node; use a compact top-to-bottom layout for this baseline and confirm the terminal node in an actual browser screenshot.
