...
`SemanticReleaseClient::diff` admits a Published+Authoritative current release and a predecessor that is either Published+Authoritative or governed Superseded+Superseded. It reports deterministic sorted concept additions/removals. This read-only comparison does not establish a supersession relation or authorize use of the predecessor. If one stable `release_id` names conflicting immutable release content, diff fails closed instead of representing the conflict as ordinary evolution. Exact concept resolution remains deterministic and performs no fuzzy matching or model call.
