...
`conceptweave-zotero --review-progress /tmp/REPORT.json /tmp/WORKSHEET.json /tmp/PROGRESS.json` reuses the same private input/output boundary and canonical worksheet comparison for an incremental checkpoint. It accepts blank decisions, counts only explicit non-abstention steward decisions, rejects missing, extra, reordered, shifted, or tampered decisions, and emits only the library/rule/digest coordinates, total, decided, remaining, and `complete`. `complete` requires a nonempty fully decided worksheet and is coverage evidence only; no authority verifier or Zotero call runs.
