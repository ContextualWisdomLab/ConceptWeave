...
The current port repair makes the remaining budget representable and preserves it across authorization; it does not claim that a concrete PostgreSQL adapter or runtime conformance exists. Exact-head execution must still prove the contract before ADR 0004 can become Accepted.
