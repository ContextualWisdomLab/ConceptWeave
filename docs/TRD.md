...
`conceptweave-zotero --worksheet /tmp/REPORT.json /tmp/WORKSHEET.json` reads one live snapshot for the two owner-only outputs. Failure cleanup requires separate review for pathname races; pair creation is not an atomic publication.
