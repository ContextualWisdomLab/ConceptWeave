...
The current port repair makes exact source+immutable-policy-binding+schema+resource authorization, remaining budget, stale-binding rejection at the port seam, snapshot-side scope containment, and binding-preserving immutable receipts representable. It does not claim that a concrete PostgreSQL adapter or runtime conformance exists. Exact-head execution must still prove the contract before ADR 0004 can become Accepted.
