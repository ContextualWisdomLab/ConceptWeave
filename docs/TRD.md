...
Duplicate review is independent of subject classification. A reviewed decision set must match the exact raw-snapshot digest, cover every duplicate candidate exactly once, select a retained key already in that cluster, and pass an external governance verifier. Its manifest records item revisions, identity mappings before and after canonicalization, and the exact rollback mapping. These mappings affect only downstream identity resolution; Zotero records are neither mutated nor deleted.
