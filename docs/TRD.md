...
PR #20 retains its rollback core and adapter: mixed-server rejection precedes reads; complete current-state checks precede inverse writes; directly verified responses advance the library version. Its existing failed-response readback still infers restored or unchanged state, and its operation-slice input does not carry original-write scope or independent execution authority. These are open repair findings, not authorized recovery contracts. Preserve original rollback functionality while repairing those boundaries.
