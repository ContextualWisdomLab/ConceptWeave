...
Source-resolution admission is now a typed Research Intake contract. `prepare_source_resolution_review` accepts only one decision per pending key and checks the exact item version, item type, parent key, and library coordinates retained in the report. Each decision records an explicit non-authoritative outcome: retain standalone evidence, require separately authorized rebind, or exclude from research scope with a reason. Missing, unknown, duplicate, stale, or blank-reason decisions fail closed. The aggregate does not mutate Zotero or claim paper-review completion; downstream review owners must bind it alongside bibliographic decisions and preserve the separate full-text capture boundary.
