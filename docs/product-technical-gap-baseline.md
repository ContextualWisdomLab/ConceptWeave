...
This refresh adopts the procedural self-application sequence through semantic projection, strict Rust transport admission and the first canonical Draft 2020-12 transport-to-domain mapper. Review `5169924981` identified that the Rust validator assumed strict JSON had already been established outside the production-language boundary. RED scaffold `b8f9e68532ba3a286c3e2d1a9ba3817e8e080ca9` plus regression `07efa642ade33af2bcd37aa270c93a2dc5d7d6f8` made duplicate decoded members, malformed strings/surrogates/trailing data, depth/size bounds and fixed diagnostics explicit; source repair `1100972a1bfbe106d877a40a4fd38732cbe5d969` implemented the std-only recognizer and test successor `04bc5c28865c40db2c67c5f8e568ff679f304cb2` broadened grammar/escape/boundary cases. Review `5170467082` then found that an `&str`-only entry point could not own the wire-size/UTF-8 boundary. RED `a6c10bfd9219736c325c8195059c2e978e8f491a`, source repair `d4a89af78153137f74eed1e6a6a2b64580b4d4a1`, and corrected test successor `ce6b064abeb1db2184d7ecf6ea2d4756d500bbb6` require raw borrowed bytes, size admission before UTF-8 validation, malformed UTF-8 fail-closed behavior and no lossy decode. Review `5171000125` then found that the Rust projection collapsed the canonical schema's optional `semantic_refs` field: bare `&[]` represented both valid absence and invalid present-empty input even though Draft 2020-12 requires `minItems: 1` when that field is present. RED contract `f9534b3a98341ff57505f0fc49529caf3b9f26ff`, production repair `7c607d8691fbe045dc419d21b2aff4777bad7043`, and compatibility-test successor `d48e2731b390ae26bcba484cdafd087dc1d13b92` preserve absence as `None`, presence as `Some(slice)`, and reject `Some(&[])`. Review `5171563060` then made the remaining schema-mapping gap executable: RED `d8e93f752801967c6708bfbea69253db459e811e` admits strict transport but intentionally cannot map a canonical valid draft, while source/test successor `e779caeab6bd0e3a1bf26ef834e3a3b2a3677306` materializes one bounded decoded tree and maps the repository-owned canonical shape before domain validation. Native Rust/Product acceptance remains unproven for these Rust lineages.
...
- Procedural self-application #44: current branch successor of transport/presence work and mapping RED `d8e93f752801967c6708bfbea69253db459e811e` -> source/test `e779caeab6bd0e3a1bf26ef834e3a3b2a3677306`, OPEN Draft on #43. It adds inferred self-application profiles, strict checked-in artifact/provenance admission, an unpublished Rust semantic/topology projection, strict raw-byte JSON admission and a private canonical Draft mapper. None of these promote parsed or referenced coordinates to authority.
...
The checked-in profiles remain inferred Draft artifacts, KO/EN authored labels only, with runtime activation disabled. Shape, provenance, transport recognition, schema mapping, semantic projection or topology validation never grants execution or publication authority.

Twelve defects now have explicit RED/finding -> repair lineages on #44:
...
9. **The production-language validator assumed strict transport parsing had happened elsewhere.** Review `5169924981` -> RED scaffold `b8f9e68532ba3a286c3e2d1a9ba3817e8e080ca9` -> regression `07efa642ade33af2bcd37aa270c93a2dc5d7d6f8` -> source repair `1100972a1bfbe106d877a40a4fd38732cbe5d969` -> test successor `04bc5c28865c40db2c67c5f8e568ff679f304cb2`. The private std-only recognizer enforces strict JSON grammar, bounded nesting, duplicate member equality after escape decoding, and fixed diagnostics. This lineage is source-shaped only until native Rust execution is obtained.
10. **An `&str`-only transport entry point could not own the wire boundary.** Review `5170467082` -> RED `a6c10bfd9219736c325c8195059c2e978e8f491a` -> production repair `d4a89af78153137f74eed1e6a6a2b64580b4d4a1` -> corrected boundary test successor `ce6b064abeb1db2184d7ecf6ea2d4756d500bbb6`. The owner seam accepts borrowed `&[u8]`, rejects payloads over 2 MiB before UTF-8 validation, rejects malformed UTF-8 without replacement/lossy conversion, then runs the strict recognizer. The `&str` convenience path delegates to the byte-first seam. Diagnostics remain fixed and non-echoing; no public/export or authority boundary widened.
11. **The Rust projection collapsed optional-field absence with schema-invalid present-empty `semantic_refs`.** Review `5171000125` -> RED/source contract `f9534b3a98341ff57505f0fc49529caf3b9f26ff` -> production repair `7c607d8691fbe045dc419d21b2aff4777bad7043` -> compatibility-test successor `d48e2731b390ae26bcba484cdafd087dc1d13b92`. `ProcedureNodeView.semantic_refs` is `Option<&[ArtifactReferenceView]>`; `None` is genuine absence, `Some` is field presence, and `Some(&[])` fails `CollectionLimit` before artifact-reference validation. This preserves the Draft 2020-12 `minItems: 1` distinction without resolving artifact authenticity, ACL or capability.
12. **Strict transport was not canonical Draft schema admission.** Review `5171563060` -> behavioral RED `d8e93f752801967c6708bfbea69253db459e811e` -> source/test successor `e779caeab6bd0e3a1bf26ef834e3a3b2a3677306`. The private ingress now materializes a bounded decoded JSON tree after strict byte admission, rejects unknown/missing/wrong-typed fields, wrong root constants/enums, invalid identity/digest/locale shapes, schema array bounds/uniqueness, schema-invalid present-empty `semantic_refs`, and missing `tool_contract_ref` for `tool_operation`, then builds the borrowed projection and invokes semantic/topology validation. Tests carry valid canonical root/edge/tool/semantic-ref witnesses plus unknown root/node/edge, constants/types, locale/enum and scope-mismatch cases. This source remains unaccepted until native Rust evidence exists.

The Rust modules remain deliberately unpublished. The byte-first transport seam and canonical mapper are now source-shaped, but no exact-current native Rust 1.98 fmt/strict Clippy/tests/rustdoc/release/owned coverage or hosted Product acceptance has materialized. The mapper's fixed `schema_invalid` result is deterministic shape admission only. It does not authenticate the caller-supplied scope, source signer or referenced artifacts, resolve released semantic/tool-contract ACL/capability, evaluate prose, approve a candidate, publish a release or authorize execution. Do not promote source-shaped RED/repair lineage into exact-head GREEN.

The next procedural source gap is **authenticated scope/revision admission around the mapped Draft**. `ProceduralScope` currently compares model/tenant/task/domain-owner strings supplied by the caller; equality is deliberately not authentication, and the Draft schema itself has no base/revision coordinate. The next owner boundary must therefore obtain tenant/task/base/revision from an independently authenticated request/revision context (Keyverse-backed identity at the application boundary), bind it to the exact candidate/revision contract without opportunistically copying values from the draft, and reject stale/mismatched base or revision before any governed transition. Released semantic/tool-contract authenticity, ACL/capability resolution, independent evaluation, retained rejected revisions, steward decision binding, immutable/CAS publication, revocation/rollback and released Noema projection remain separate later gates.
...
4. Restack dependent #5/#9/#43 and descendants normally and reset evidence. #44 preserves all twelve repair lineages but does not inherit acceptance from any predecessor head.
5. On #44, obtain native Rust 1.98 evidence for the byte transport, semantic-presence and canonical mapping seams before widening/exporting them; then implement independently authenticated tenant/task/base/revision admission without moving Keyverse identity truth or foreign artifact authority into ConceptWeave.
