...
- #46 `codex/pr6-v3-index-evidence`, OPEN Draft, is the active Source Observation writer. The current lineage contains PostgreSQL 18 first-class NOT NULL evidence, signed-`int2` `coninhcount`, corrected PRIMARY KEY completeness, relation-kind-specific NOT NULL enforcement state, fail-closed table-like relation ownership, relation-wide `pg_constraint` name integrity across the standard and NOT NULL evidence families, partition `NO INHERIT` rejection, parent-row/column resolution, parent-graph acyclicity, direct declarative-partition parent validation against an independently captured `pg_inherits` witness, fail-closed `inhdetachpending` handling, PostgreSQL-compatible parent/child `convalidated` asymmetry, fail-closed inheritance-origin coherence for `conislocal=false / coninhcount=0`, and foreign-table validation-state integrity. Review `5196715571` isolates the foreign-table `NOT VALID NOT NULL` gap; executable RED `06839f048c4c01a6164aaaf61b5cd861f8b06783`, production repair `f715dbff95df880f63b28a074a0cf711dfa14ddc`, and doctoring `d580256d5339811851685e955bc6d7a45dd6d05c` preserve the current causal lineage. Exact-head native and hosted acceptance remain pending.
...
- foreign-table NOT NULL validation integrity: `5196715571 -> 06839f04... -> f715dbff... -> d580256d...`;
...
`NotNullConstraintObservation::new()` therefore scopes the fail-closed enforcement guard to non-foreign owners. `RelationKind::ForeignTable` preserves both observed `conenforced` values, and the existing digest already hashes the bit. The focused regression constructs both states as bounded foreign-table snapshots and proves their digests differ. The adapter must capture the catalog value; it may neither normalize it to true based on ordinary-table semantics nor synthesize false from the general fact that core PostgreSQL does not enforce foreign-table constraints.

### Foreign-table validation integrity

PostgreSQL 18 deliberately gives foreign-table `NOT NULL` a different validation state space from ordinary-table `NOT NULL`. `ALTER FOREIGN TABLE ... ADD table_constraint [ NOT VALID ]` supports both CHECK and NOT NULL constraints but states that `NOT VALID` is allowed only for CHECK. `CREATE FOREIGN TABLE` likewise exposes enforcement state for NOT NULL but no NOT VALID form.

`NotNullConstraintObservation::new()` therefore rejects `RelationKind::ForeignTable && convalidated=false` as `not_null_constraint_foreign_validation` before governed identity construction. This does not undo PostgreSQL 18's ordinary-table NOT VALID support and does not conflate validation with enforcement: foreign-table `conenforced` still preserves both source-supported values. The adapter must capture both fields independently and fail closed if a foreign-table NOT NULL row reports an unsupported validation tuple rather than normalizing or hashing it.
