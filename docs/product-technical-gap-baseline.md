...
- #46 `codex/pr6-v3-index-evidence`, OPEN Draft, is the active Source Observation writer. The current lineage contains PostgreSQL 18 first-class NOT NULL evidence, signed-`int2` `coninhcount`, corrected PRIMARY KEY completeness, fail-closed NOT NULL enforcement for every supported owning relation kind, fail-closed table-like relation ownership, relation-wide `pg_constraint` name integrity across the standard and NOT NULL evidence families, partition `NO INHERIT` rejection, parent-row/column resolution, parent-graph acyclicity, direct declarative-partition parent validation against an independently captured `pg_inherits` witness, fail-closed `inhdetachpending` handling, PostgreSQL-compatible parent/child `convalidated` asymmetry, fail-closed inheritance-origin coherence for `conislocal=false / coninhcount=0`, and foreign-table validation-state integrity. Review `5196804677` corrects the superseded foreign-table enforcement interpretation using REL_18_STABLE implementation authority; RED `4a611cc3dc15f4200f65bf08945c9cfa759daa7c`, production repair `3e9828311a55187d758b9bf729f69870c16f7be9`, enforcement doctoring `632f678a0d8b83ebcec8c02bc36481b2495e06d4`, and validation-doctoring alignment `141a632ffd1dbabaa3a41cccc833718eaa63d407` preserve the current causal lineage. Exact-head native and hosted acceptance remain pending.
...
`convalidated=false` remains a legitimate first-class PostgreSQL 18 NOT NULL state for ordinary-table paths and stays in governed identity there. The parent-edge rule is narrower. PostgreSQL `REL_18_STABLE` `AdjustNotNullInheritance()` rejects an existing child NOT NULL constraint that is still NOT VALID when the desired inherited parent constraint is valid, while explicitly allowing the opposite direction. Canonicalization therefore rejects `parent.validated() && !child.validated()` as `not_null_constraint_parent_validation` only after the exact parent row and corresponding column have resolved. A NOT VALID parent with an already-valid child remains accepted where that source state is supported.

This preserves source semantics rather than turning validation into a symmetric equality check. The executable regression contains both directions so a future refactor cannot “simplify” the asymmetric PostgreSQL rule into an invalid global prohibition on ordinary-table `NOT VALID`.
...
### NOT NULL enforcement integrity

PostgreSQL 18 REL_18_STABLE is authoritative for source-representable catalog state. `StoreRelNotNull()` records NOT NULL cooked constraints with `is_enforced = true`, and `CreateConstraintEntry()` asserts that only CHECK or FOREIGN KEY constraints may be not enforced. Therefore `pg_constraint.contype='n'` with `conenforced=false` is invalid for ordinary, partitioned, and foreign tables alike.

The earlier foreign-table exemption derived from the generic placement of `[ ENFORCED | NOT ENFORCED ]` in the `CREATE FOREIGN TABLE` synopsis is explicitly superseded. The documentation note that PostgreSQL core does not enforce foreign-table CHECK/NOT NULL declarations describes runtime trust in the remote system, not a wider local NOT NULL `conenforced` catalog domain.

`NotNullConstraintObservation::new()` now rejects every NOT NULL observation with `enforced=false` as `not_null_constraint_enforcement`. The adapter still captures the source bit independently and fails closed on false; it must not normalize, synthesize, or infer it from remote behavior.

### Foreign-table validation integrity

PostgreSQL 18 gives foreign-table `NOT NULL` a narrower validation state space than ordinary-table `NOT NULL`. `ALTER FOREIGN TABLE ... ADD table_constraint [ NOT VALID ]` supports CHECK and NOT NULL constraints but states that `NOT VALID` is allowed only for CHECK. `CREATE FOREIGN TABLE` likewise has no NOT VALID form for NOT NULL.

`NotNullConstraintObservation::new()` therefore rejects `RelationKind::ForeignTable && convalidated=false` as `not_null_constraint_foreign_validation` after the universal NOT NULL enforcement guard. This does not undo PostgreSQL 18's ordinary-table NOT VALID support. Foreign-table NOT NULL requires both captured `conenforced=true` and `convalidated=true`; the two fields remain separate source facts and are not inferred from one another.
