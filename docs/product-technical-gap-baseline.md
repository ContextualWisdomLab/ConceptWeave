...
A subsequent finding `5204038985` mistakenly imported a later PostgreSQL development-line `PG_UNUSED_1` hole into the PostgreSQL 18 contract. Fresh authoritative review `5204368970` against `REL_18_STABLE/src/include/mb/pg_wchar.h` established that PostgreSQL 18 still defines encoding ID `7` as `PG_MULE_INTERNAL`, while `PG_VALID_BE_ENCODING` is simply the inclusive `0..=PG_KOI8U` backend range. The superseded RED `365d189d14e2525774bb32657ea6b4127a105ee9` and repair `d438f8a77df9c892aa4bc65f62f65ad857bb7ea6` remain historical commits but are not current authority.

Corrective RED `a3f877a65105dc7304d524e2fced2f65de83d6d7` requires PostgreSQL 18 ID `7` to be accepted while client-only `35` and negative sentinel `-1` remain rejected. Minimal production repair `15471cf7cb3639f2258b0d39950919fd9f7fad65` removes the future-version hole exclusion and documents the version-specific backend range without changing the database-encoding successor digest framing. Decision record: `docs/doctoring/postgresql-index-collation-database-encoding-integrity.md`.
...
2. Obtain one unchanged #46 representation head with repository-pinned Rust 1.98 native GREEN plus applicable hosted Product/security/dependency/review terminal GREEN. The catalog-identity and PostgreSQL-18-specific database-encoding contracts are part of this gate.
3. Only after that representation gate, extend #46 ordinary-forward with the concrete PostgreSQL 18 semantic-expression extractor/live attached-index differential oracle and raw `pg_attribute.atttypmod` adapter/rowtype differential. The same bounded source observation must capture `pg_database.encoding`, accept the exact PostgreSQL 18 backend domain including `PG_MULE_INTERNAL == 7`, reject client-only IDs, resolve nonzero collation OIDs to exact `(namespace, name, collencoding)` rows, and prove each resolved row is `-1` or that exact database encoding. A future PostgreSQL major that retires an encoding ID requires its own versioned contract rather than mutation of the v18 representation.
