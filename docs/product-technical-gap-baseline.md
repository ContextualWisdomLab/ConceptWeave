...
- Pending-source resolution #40: latest source-bearing predecessor `8445cbb6ce5fc6988ab4609d290685ddb2fd3f01`, OPEN Draft/mergeable on #9; the live PR ref is authoritative for later docs-only successors. Reality RED `a3cd9b0d68b4ecc28322860d04a23867412d611f` proves internal duplicated identity is not an external immutable-report binding. Causal repair `453878f236cd46ae5389575719f37077302da70e` removes direct deserialization into the trusted review and restores stored artifacts only by reconstructing against the caller-supplied immutable `ClassificationReport`; test/source successor `f660c3a95c19dbb568faf7e301189451bc6ecd36` explicitly rejects report metadata, pending-key and expected-identity drift, and `8445cbb...` removes only a redundant post-constructor identity comparison while retaining wire-to-decision and decision-to-report binding. Status is `SOURCE_TEST_REPAIRED_PENDING_CI`, not executable GREEN. #40 owns only the typed read-only exact-snapshot resolution aggregate; it has no Zotero write, semantic publication or approval authority.
