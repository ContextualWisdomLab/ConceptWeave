...
### Partially repaired systemic PostgreSQL identifier boundary
...
The legacy shared model is now repaired rather than merely listed as residual work. Structural RED `3136e841ed925deffa904f578acc4e46c80bb763` covers legacy column/table/constraint/foreign-key/location identifiers and includes negative controls for rendered data-type and CHECK-definition text. Production repair `b4068f5e78de4f5e76ca1eef70f17cce68b83c1c` routes only those identifier-bearing fields through `validate_postgresql_identifier`; the generic `validate_nonblank()` function remains unchanged. Review `5257904564` records the inspected predecessor finding after the commits were created and is explicitly not treated as evidence that review preceded the RED/fix sequence.

The remaining active RED is now concentrated in `representation_v3.rs`. Existing systemic contracts plus v3 catalog structural RED `5ed597aab1e0770e9702e766beeb78c3aa03c84d` require exact identifier handling for qualified type/collation/operator-class names, v3 column/relation/domain/enum/index/location coordinates, domain constraint names, and simple index-column attributes. `IndexTablespace` is also an identifier-bearing catalog family: PostgreSQL exposes the tablespace name as object name `spcname`, and `CREATE TABLESPACE` accepts a tablespace name subject to object-name rules plus the `pg_` reserved prefix restriction. Contract movement `fe2d3f01efae5fdad03b39a785ffd096f5d91fdd` therefore removes the previous false assumption that whitespace-only tablespace names are presentation-invalid and instead requires exact whitespace preservation plus empty/NUL rejection.

This is meaningful production progress but **not systemic GREEN**. The remaining repair must route the affected `representation_v3.rs` identifier fields to the PostgreSQL-identifier admission rule while keeping rendered type text, reconstructed CHECK definitions, rendered index expressions/predicates, extractor revision, operator/storage option text, and other non-identifier metadata on their existing field-specific policy. Globally weakening `validate_nonblank()` remains prohibited. Exact rationale and lineage are current in `docs/doctoring/postgresql-identifier-fidelity-systemic-integrity.md`.
...
**The current Source Observation head is RED / not GREEN.** The systemic identifier repair remains incomplete until the remaining `representation_v3.rs` identifier call sites are causally repaired. The final exact head must then independently demonstrate repository-pinned Rust 1.98 `fmt`, strict workspace/all-target Clippy, focused and retained tests, workspace/doc tests, release build, rustdoc, owned production statement/branch/edge coverage, and the bounded PostgreSQL 18 same-generation differential. No predecessor execution result transfers after source or documentation movement.
...
- legal quoted identifier content, including whitespace-only schema/relation/table/column/constraint/reference/type/collation/operator-class/tablespace and converter role/schema/function names, survives observation byte-for-byte rather than being rejected or trimmed;
