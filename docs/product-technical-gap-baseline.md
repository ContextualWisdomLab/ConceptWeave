...
Golden-set #10 was stale on historical #9 `51c7df6...`. Ordinary two-parent integration `c9954e286041c80d08655e092f10007a684ceab1` adopts both historical #10 and current #9 without force or destructive rebase. The repaired architecture no longer modifies `ZoteroItem` or `ClassificationReport`: provider-level evidence enters through private `CapturedZoteroItem`; `GoldenSnapshot` layers immutable review evidence over the trusted report; evaluation uses read-only accessors; provider-captured, typed-fixture and proposal digests use distinct v3 domains. Historical tests that required public trusted-state mutation were replaced with externally reachable invalid-input/approval cases.

This is source/stack repair, not acceptance. The reconciled branch requires fresh exact-head Rust 1.98 workspace/fmt/strict Clippy/rustdoc/release/owned coverage, hosted checks and qualifying independent review. #11 and later research descendants must stay open and adopt the accepted #10 successor by ordinary non-force integration rather than copying selected files.
...
| Golden-set evaluation | STACK_RECONCILED_PENDING_ACCEPTANCE | #10 ordinary-adopted current #9 and repaired aggregate/raw-capture compatibility; Rust/hosted/review evidence is not yet established. |
