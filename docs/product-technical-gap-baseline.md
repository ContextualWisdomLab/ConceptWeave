...
- foreign-table NOT NULL enforcement correction: `5196207955 -> 6d2b2289... -> 41de3e39... -> 0b401718...`;
...
`NotNullConstraintObservation::new()` therefore scopes the fail-closed enforcement guard to non-foreign owners. `RelationKind::ForeignTable` preserves both observed `conenforced` values, and the existing digest already hashes the bit, so enforced and unenforced foreign-table source states do not collapse. The adapter must capture the catalog value; it may neither normalize it to true based on ordinary-table semantics nor synthesize false from the general fact that core PostgreSQL does not enforce foreign-table constraints.
