...
- Pending-source resolution #40: latest source-bearing predecessor `e72fb404375e7a29489a855069fa0150026888e8`, OPEN Draft/mergeable on #9; the live PR ref is authoritative for later docs-only successors. Reality RED `a3cd9b0d68b4ecc28322860d04a23867412d611f` proves internal duplicated identity is not an external immutable-report binding. Causal repair `453878f236cd46ae5389575719f37077302da70e` removes direct deserialization into the trusted review and restores stored artifacts only by reconstructing against the caller-supplied immutable `ClassificationReport`. Status is `SOURCE_TEST_REPAIRED_PENDING_CI`, not executable GREEN. #40 owns only the typed read-only exact-snapshot resolution aggregate; it has no Zotero write, semantic publication or approval authority.
...
`SourceResolutionReview` constructor admission rejects missing/blank report server identity, wrong server/library/item identity, duplicate/unknown decisions, missing decisions, blank reasons and missing retained inventory. Stored JSON cannot regain exact-snapshot trusted status from internal agreement alone; restoration must preserve the same immutable-report binding as constructor admission.
...
- Coordinated-rewrite RED `a3cd9b0d68b4ecc28322860d04a23867412d611f` changes both `expected_source_identities[0].item_version` and `resolved_sources[0].item_version` together, proving that two mutable copies in the same JSON are not provenance. Causal repair `453878f236cd46ae5389575719f37077302da70e` makes `SourceResolutionReview` serialize-only, decodes persisted JSON to private `StoredSourceResolutionWire`, reconstructs a candidate through `prepare_source_resolution_review(report, ...)`, and rejects stored envelope/expected identity coordinates that differ from that immutable-report-bound reconstruction. Ordinary merge `e72fb404375e7a29489a855069fa0150026888e8` preserves the concurrent baseline lineage without force.
- `docs/doctoring/zotero_source_resolution_item_identity.md` records this lane as `SOURCE_TEST_REPAIRED_PENDING_CI`. Historical docs successors remain evidence, not execution proof.
...
| Pending-source resolution | SOURCE_TEST_REPAIRED_PENDING_CI | #40 rejects missing provider identity, nested server/library drift, duplicate/unsorted/blank stored decisions, blank stored rule revision, missing decisions and unilateral item-identity drift. Coordinated expected/decision identity rewriting is now rejected through report-bound restoration rather than internal JSON agreement. Exact-head Rust/coverage/hosted/independent-review evidence is still required. |
