...
- PostgreSQL 18 first-class NOT NULL identity, aggregate admission, partition-parent validity, `coninhcount` source-domain bound, and PRIMARY-KEY-backed `attnotnull` separation: `5189886354 -> 8ce7fd7c... -> 5c0706da... -> 8912039b... -> e58dd0c5... -> 5ac4cf33... -> 91086655... -> 5190108906 -> 4c1c0640... -> 2f8a4f97... -> e1c62c07... -> c5ac66ff... -> e60cfec3... -> 5190362750 -> 28916758... -> 385bf437... -> 1b16ce65... -> 5190456512 -> 4236309b... -> 5a29f0aa... -> ca8d8e5a...`.
...
Exact-source review `5190456512` then found a semantic false-positive in the family completeness rule: `attnotnull=true` does not imply that an explicit `contype='n'` row exists, because PostgreSQL's catalog-not-null model permits the PRIMARY KEY itself to be the backing constraint. RED `4236309b5317f54a38d39996affd5b7a950b9f06` replaces the incorrect 'every non-nullable column has an explicit not-null row' test with a PK-backed non-nullable column and an observed-empty explicit NOT NULL inventory. Repair `5a29f0aa2d0a07ef1ef3182326efb4e70ecfe5a2` removes only the invalid reverse-set equality while retaining coordinate resolution, duplicate explicit-row rejection, relation-local name uniqueness, and the forward invariant that every explicit `contype='n'` row targets a known non-nullable column. Doctoring `ca8d8e5a7ddf7971a18cea1e5714a523d94d444e` records the corrected source semantics and adapter obligation.
...
PostgreSQL 18 documentation states that `pg_constraint` stores not-null and primary-key constraints, and PostgreSQL developer evidence for the catalog-not-null implementation states that an `attnotnull` flag can be backed by either a not-null constraint or a primary-key constraint. Adapter completeness therefore means complete capture of the bounded `contype='n'` query result, not equality between explicit not-null rows and every `attnotnull=true` column.
...
- Herrera, A. (2024). *Re: Can't find not null constraint, but \\d+ shows that* [PostgreSQL hackers message]. PostgreSQL. https://www.postgresql.org/message-id/202404120752.6ebv4q5zwnfw%40alvherre.pgsql
