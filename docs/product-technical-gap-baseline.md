...
Finding review `5185362949` identified the first observed-family composition defect after the temporal type-kind repair. A snapshot produced by `new_with_array_types(...)` could carry an exact custom true-array coordinate such as `public._status`, but a later `with_observed_type_kinds(...)` call revalidated relation/domain bindings without the already-authoritative `ArrayTypeObservation` inventory. Behavioral RED `ef9c8d61d0864e7b867902746969ea4d33592407`, refined at `1cd78d9a9561aea9dc6d980bee59f31b9086b0d9`, established that exact array evidence and coherent type-kind evidence must compose. Production repair `e96367b2944ce3bb33f2b33be04dd36bb0f84e33` made the consuming attachment path preserve those exact observed array coordinates; source review `5185490815` verified that repair.

Follow-up review `5185657110` found that the initial contract over-corrected the opposite boundary. `PostgresTypeKind::Base` is source-authoritative `pg_type.typtype='b'` evidence for an exact type row, so an ordinary user-defined Base type must be a resolvable qualified type binding. Base classification does **not**, however, prove that the row is the true array for another type; PostgreSQL exposes that separately through reciprocal `typarray`/`typelem` evidence. Treating Base-only binding success as forbidden therefore conflated type-coordinate existence with array identity.
