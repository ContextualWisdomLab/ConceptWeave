...
| Client Consumption | ACTIVE_CHILD | Offline Published+Authoritative admission, compatibility, exact resolution/diff, canonical digest validation, `verify_detached_artifact` byte-integrity verification and explicit supersession validation exist. Public supersession/publication schema/fixtures remain the next Client contract lane after Foundation terminal quality. |
