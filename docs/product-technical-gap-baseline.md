...
- #46 `codex/pr6-v3-index-evidence`, OPEN Draft, is the active Source Observation writer. The current lineage contains PostgreSQL 18 first-class NOT NULL evidence, signed-`int2` `coninhcount`, corrected PRIMARY KEY completeness, enforced-only NOT NULL state, partition `NO INHERIT` rejection, parent-row/column resolution, parent-graph acyclicity, direct declarative-partition parent validation against an independently captured `pg_inherits` witness, and fail-closed `inhdetachpending` handling. Direct-parent review `5194258395` is followed by RED `51a8f2680943f2526b1f03712ebf92f5cad30c10`, production `4cbb426d045f290a061820593fee0a870c7cbd94`, retained-contract alignment `d7c11072ab1c03d928129b547be9b0d608fc8039`, and edge coverage `610a557526f85f456bdd8c8c1e28fe184b3a79f0`. Follow-up review `5194302440` identified discarded detach state; source repair `ee12c67716de0c617f40f9f75bda3ae2528b639f`, retained fixture updates `e4d33a9dceecea57c88dfd0a5b848be2f05921de` / `4359936e2a32566428b8eaafdeee97386152d509`, detach regression `eaa354bb6d0d0eee72753bf868bb8881ba5e6a0b`, and doctoring `5f55729965dd8e2317289a3eea0d4cd180ceb425` preserve the causal lineage. Exact-head native and hosted acceptance remain pending.
...
- `.github#2106@44901e45636e655cf84cc609e5fe62789216cde9` is OPEN Ready / mergeable. Runtime Quality `34794865763`, Semgrep `34794865812`, and Python Security `34794865728` are success; CodeQL `34794865762` and Security Scan `34794865771` remain non-terminal.
- `.github#2170@d493cc4c53d0b77d67ef4af13005dcda8fb33c7f` is OPEN Ready / mergeable. Runtime Quality `34794949744`, Semgrep `34794949745`, and Python Security `34794949775` are success; Security `34794949825` and CodeQL `34794949758` remain queued.
- `.github#2079@2d27e0c13f9b118ca844f5299b0fcdde420fa66b` is OPEN Ready / mergeable. Semgrep `34794916614`, Python Security `34794916564`, and Security `34794916586` are success; CodeQL `34794916559` remains non-terminal.
...
For PostgreSQL 18 NOT NULL constraints, the adapter captures `pg_attribute.attnotnull`, bounded `pg_constraint.contype = 'n'` rows, referenced parent constraints, owning `pg_class` rows, and the child relation's direct `pg_inherits` edge inside the same catalog snapshot. `conkey` resolves to exactly one bounded column. A nonzero `conparentid` requires a real parent-constraint join, parent `pg_class.relkind = 'p'`, `conislocal=false`, `coninhcount=1`, `connoinherit=false`, corresponding constrained columns, an acyclic parent-constraint graph, and an independent direct `pg_inherits` witness whose `inhrelid` is the child relation, `inhparent` is the same parent relation that owns the resolved parent constraint, and `inhdetachpending=false`. The adapter must not manufacture the witness from `conparentid`, caller text, names, or column shape, nor default a missing detach-state value to `false`. Missing, unauthorized, contradictory, duplicate, cross-column, cyclic, `NO INHERIT`, unenforced, detach-pending, or partially resolved evidence fails closed before immutable snapshot construction.
