...
- Focused doctoring: `c9aff9a8f655e868ccddf09fe58b7aaf5bd113ef`, `docs/doctoring/postgresql-index-constraint-inheritance-state-integrity.md`.
...
No executed Rust RED/GREEN or hosted Product acceptance is claimed after the current ordinary-forward head movement. One unchanged exact #46 representation head must pass repository-pinned Rust 1.98 `fmt`, strict workspace/all-target Clippy, the exact constraint-parentage and inheritance-state contracts plus every retained Source Observation/relation-partition focused contract, workspace/doc tests, release build, owned production rustdoc/test/edge-case coverage, and applicable hosted Product/security/dependency/review gates. Any head movement resets exact-head acceptance.
...
3. Only after that representation gate, extend #46 ordinary-forward with concrete PostgreSQL 18 extractor/live differentials. The constraint differential must verify `pg_constraint.conparentid`, `conislocal`, and `coninhcount` together after successful constraint-backed index attachment; reject missing/wrong parentage or impossible local/count state in governed successors; and preserve `(conparentid=0, conislocal=true, coninhcount=0)` when a constraint-backed child index is attached below a non-constraint parent index. All retained child-constraint, child-validity, foreign-table, direct-partition collation/declaration, database-encoding/collation/version, expression, and `atttypmod` differentials remain required.
