...
This refresh adopts the intervening procedural self-application sequence `0fdbf5d304c0fdb8d6faa0c0bcc44f54490fad12` -> semantic RED `564ea965e2dcf89b84af66d646ad2b7c6b9d1889` -> semantic projection implementation `502e2719877a5d2945a86faaa7c9f557729280d3` -> topology-fixture successor `8029153a86d1bfa959064ce0a6eddd7b928c552f` -> test-contract documentation repair `be82c216e51669ef48743aad5864ebe278d78ac8`. The Rust candidate now preserves the schema-significant procedural fields that the earlier topology-only view dropped. Native Rust/Product acceptance is still unproven on these newer heads.
...
- Procedural self-application #44: current branch successor of executable source/test `8029153a86d1bfa959064ce0a6eddd7b928c552f`, OPEN Draft on #43. It adds inferred self-application profiles, strict checked-in artifact/provenance admission, and an unpublished Rust semantic/topology projection that now preserves `procedure_kind`, eight locale slots, bounded semantic/tool-contract coordinates and edge condition/guidance/pitfalls without promoting those coordinates to authority.
...
The checked-in profiles remain inferred Draft artifacts, KO/EN authored labels only, with runtime activation disabled. Shape, provenance, semantic projection or topology validation never grants execution or publication authority.

Eight defects now have explicit RED/finding -> repair lineages on #44:
...
The Rust module remains deliberately unpublished and borrowed-view-only. It still does not parse transport JSON or authenticate the expected scope. No hosted workflow run has materialized for the newer Rust heads in the current observation, and native Rust 1.98 fmt/strict Clippy/tests/rustdoc/release/owned coverage are not established here. Do not promote source-shaped RED/repair lineage into exact-head GREEN.

The next procedural source gap is a production Rust transport/domain adapter that preserves duplicate-decoded-key fail-closed behavior, maps the canonical Draft 2020-12 shape without losing schema-significant or absent/present distinctions, and compares candidate scope against independently authenticated tenant/task/base/revision context. Released semantic/tool-contract authenticity, ACL/capability resolution, independent evaluation, retained rejected revisions, steward decision binding, immutable/CAS publication, revocation/rollback and released Noema projection remain separate later gates.
...
4. Restack dependent #5/#9/#43 and descendants normally and reset evidence. #44 preserves all eight repair lineages but does not inherit acceptance from any predecessor head.
5. On #44, progress production Rust ingress/domain admission from executable REDs; do not repeat the already-repaired semantic-field projection or expand the JS/local-Git checkers without a new valid finding.
