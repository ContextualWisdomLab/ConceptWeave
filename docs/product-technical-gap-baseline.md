...
- #46 `codex/pr6-v3-index-evidence`, OPEN Draft, is the active Source Observation writer. The PostgreSQL 18 first-class NOT NULL lineage includes aggregate admission, partition-parent relation-kind validation, the `coninhcount` source-domain bound, corrected PRIMARY KEY completeness, fail-closed rejection of partitioned-table `NO INHERIT`, and the partition-child `conparentid` locality/direct-ancestor tuple repair. Semantic source repair is present through `03bb42acca171fd52d8a3d9d4faea810c6ae317a`; research doctoring is current through `4f032eb8ab66c15b7bfee266fd5b49a0d4abba63`. Exact-head native and hosted acceptance remain pending.
...
Fresh PostgreSQL 18 `REL_18_STABLE` source review found that the partition-parent linkage tuple was still under-constrained. `ConstraintSetParentConstraint()` asserts `coninhcount == 0` before linkage, sets `conislocal=false`, increments `coninhcount` exactly once, and then stores `conparentid`. Review `5192015894` therefore rejects treating those fields as independent when `conparentid != 0`. RED `57c7f6c4d418a7905f50bb0526711afe1cb1c602` requires a resolved partition-parent link to reject `is_local=true` and any direct-ancestor count other than one. Production repair `03bb42acca171fd52d8a3d9d4faea810c6ae317a` applies only at `with_parent_constraint()`, leaving ordinary inheritance rows with `conparentid=0` free to carry their source-authoritative locality/count states. Doctoring `4f032eb8ab66c15b7bfee266fd5b49a0d4abba63` records the source operation, rejected alternative, and adapter obligation.
...
Protected `.github/main` advanced normally through #2172 and #2178 to `828eaaefb0cc97bba4da63eb9270447476d26710`. Those commits add and harden Noema office-document review extraction and its quality-CI coverage. They do not overlap ConceptWeave Source Observation semantics, but they are protected truth and therefore stale central feature heads were adopted ordinary/non-force rather than rewritten.

`.github#2170` preserves the bounded Required OpenCode `coverage-evidence` RCA source repair and the full-suite Noema dependency-closure repair. Current exact `ae0f2f57f1d2abda7bb2e7ac9ce3bf8f1cac6f39` is OPEN/Ready/mergeable at `SOURCE_REPAIRED / FULL_SUITE_DEPENDENCY_REPAIRED / ACCEPTANCE_PENDING`. Exact-head CodeQL PR `34775874728`, Security Scan `34775874680`, Python Security `34775874736`, Agent Review Runtime Quality CI `34775874685`, and SAST Semgrep `34775874812` remain queued. Earlier-head execution/review evidence is historical only; no manual/no-op retrigger is authorized.

The protected CodeQL handler prerequisite `.github#2106` remains exact `1ba96e4ddf6a800435651ec1c49acff533242fd9`, OPEN/Ready/mergeable on protected `main@828eaaef...`. Prior CodeQL chronology remains RCA history, not acceptance for this head; fresh central acceptance remains required.

`.github#2079` owns the Noema finding↔confirmed-probe contract. Exact `6d7e833224e06b4316df3d6bbdfcb4658e151956` remains OPEN/Ready/mergeable on protected `main@828eaaef...`. The finding/probe relation and touched-callable docstring defects are source-repaired without threshold weakening; post-Ready exact-head Security, Semgrep, CodeQL, Python Security and Required OpenCode/coverage evidence remain acceptance gates. Predecessor 13/17 coverage is historical only.
