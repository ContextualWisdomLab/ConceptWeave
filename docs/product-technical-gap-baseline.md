...
1. **Source Observation adapter** — real PostgreSQL introspection behind a port, immutable bounded receipts, domain/enum/index/comment evidence, hostile-input/resource bounds, cancellation, source-disappearance behavior, and a frozen GRC reference fixture; populate implemented PK/unique/FK/CHECK/reference-behavior/validation/enforcement contracts rather than duplicate relationship semantics in the adapter.
