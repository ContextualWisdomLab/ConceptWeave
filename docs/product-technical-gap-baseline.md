...
- #46 `codex/pr6-v3-index-evidence`, OPEN Draft, is the active Source Observation writer. Its current lineage contains the PostgreSQL 18 first-class NOT NULL family, signed-`int2` `coninhcount` bound, corrected PRIMARY KEY completeness, enforced-only NOT NULL source domain, partitioned-table `NO INHERIT` rejection, partition-child locality/direct-ancestor validation, same-family parent-row resolution, corresponding-column validation, partition-parent `NO INHERIT` rejection for nonzero `conparentid`, and acyclic partition-parent constraint topology. Latest graph-integrity review `5193997785` is followed by executable regression contract `a8ef446249ef9249d3f893a94b9d3ee7d1b36822`, production repair `2a0643952e5f3150c8b27d3a7c11c30525b6871f`, and primary-source doctoring `1d529c02b67ab6d23a79cf4d7c774b14ea446e77`. Exact-head native and hosted acceptance remain pending.
...
- partition-parent graph integrity: `5193997785 -> a8ef4462... -> 2a064395... -> 1d529c02...`.

The latest repair closes a graph-level source-integrity gap. Before `2a064395...`, every nonzero parent coordinate could resolve to a real same-column NOT NULL row while two or more rows still formed a cycle such as `metric_a -> metric_b -> metric_a`. PostgreSQL declarative partitioning forms a parent/partition hierarchy: a partition has only the partitioned table it belongs to as parent, and `pg_constraint.conparentid` identifies the corresponding constraint of that actual parent. The canonicalizer now indexes exact constraint coordinates and walks every parent chain; revisiting a row fails as `not_null_constraint_parent_cycle` before governed hashing.

The earlier enforcement repair closes an impossible source-domain state that the original contract explicitly admitted. PostgreSQL 18 exposes generic `ENFORCED`/`NOT ENFORCED` constraint syntax but documents `NOT ENFORCED` as currently supported only for foreign-key and CHECK constraints. The PostgreSQL 18 release announcement describes the same boundary: foreign keys and CHECK constraints gain `NOT ENFORCED`, while NOT NULL gains names, `NOT VALID`, and `NO INHERIT`. Therefore a caller-supplied `contype='n'` row with `conenforced=false` is contradictory evidence rather than a second valid NOT NULL identity. `NotNullConstraintObservation::new` rejects it as `not_null_constraint_enforcement`; `NOT VALID` remains valid and continues to alter governed identity.

The partition-parent attachment repair keeps ordinary `RelationKind::Table` `NO INHERIT` valid for classic inheritance but rejects it once a nonzero partition-parent coordinate is attached. PostgreSQL declarative partitioning always inherits parent NOT NULL/CHECK constraints into partitions, so `with_parent_constraint()` rejects `connoinherit=true` as `not_null_constraint_parent_no_inherit`; generic inheritance with `conparentid=0` remains unchanged.
...
PostgreSQL 18 `pg_constraint` defines `conparentid` as the corresponding parent-partition constraint, `connoinherit` as the non-inheritable flag, and `conkey` as constrained column numbers. PostgreSQL 18 partitioning rules make parent NOT NULL constraints mandatory on partitions and restrict each partition to its partitioned-table parent. `ALTER TABLE ... ATTACH PARTITION` requires the attached table to have the parent's NOT NULL and CHECK constraints without `NO INHERIT`. `REL_18_STABLE` `findNotNullConstraintAttnum()` selects a NOT NULL row by relation plus exact constrained attribute number before partition linkage. The governed edge therefore binds parent-row identity, corresponding constrained column, an inheritance-compatible child flag tuple, and an acyclic partition topology.
