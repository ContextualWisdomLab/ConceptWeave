...
- Pending-source resolution #40: latest test-bearing predecessor `a7b2588baa727ce973323c900e1d00b9c76425b2`, OPEN Draft/mergeable on #9; the live PR ref is authoritative for later docs-only successors. Earlier reality RED `a3cd9b0d68b4ecc28322860d04a23867412d611f` proved internal duplicated identity was not an external immutable-report binding, and causal repair `453878f236cd46ae5389575719f37077302da70e` replaced direct trusted deserialization with report-bound restoration. Fresh strict-wire RED `a7b2588...` now proves the private stored wire and its nested decision/identity DTOs still accept undeclared JSON fields and silently discard them before returning the trusted review. Status is `REALITY_RED_SOURCE_FIX_PENDING`, not executable GREEN. #40 owns only the typed read-only exact-snapshot resolution aggregate; it has no Zotero write, semantic publication or approval authority.
...
- Strict-wire RED `a7b2588baa727ce973323c900e1d00b9c76425b2` adds a public stored-artifact regression that injects an undeclared top-level `approval`, nested identity `source_digest`, and nested decision `semantic_authority`. Current Serde-derived wire DTOs ignore those fields, so the test requires fail-closed rejection before a trusted review can be reconstructed. Least-widening repair is strict unknown-field admission on the persisted wire DTO boundary; constructor/report binding, disposition semantics and Zotero authority remain unchanged.
- `docs/doctoring/zotero_source_resolution_item_identity.md` records the earlier report-binding lane as `SOURCE_TEST_REPAIRED_PENDING_CI`; the fresh strict-wire regression supersedes overall #40 status to `REALITY_RED_SOURCE_FIX_PENDING` until its source repair exists.
...
Current #40 status is **REALITY_RED_SOURCE_FIX_PENDING**, not GREEN. The latest pinned predecessor coverage run reports 173/182 branches (95.05%), 1,773/1,808 lines (98.06%), and 188/192 functions (97.92%); those metrics do not transfer to the changed test/docs head and remaining gaps are recorded without exclusions. Keep Draft and keep valid review findings unresolved until one unchanged exact successor completes the full Rust/coverage/hosted/independent-review gate.
...
| Pending-source resolution | REALITY_RED_SOURCE_FIX_PENDING | #40's report-bound restoration rejects missing provider identity, nested server/library drift, duplicate/unsorted/blank stored decisions, blank stored rule revision, missing decisions and item-identity drift. Fresh strict-wire RED proves undeclared stored fields are still silently accepted; source repair plus exact-head Rust/coverage/hosted/independent-review evidence is required. |
