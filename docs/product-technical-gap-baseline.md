...
Review `5257069180` on predecessor exact `427587e8be39d38240e57a2ebcc9cdaf291f6ebe` established that the converter-specific repair did not cover the shared Source Observation representation. `model::validate_nonblank()` uses `value.trim().is_empty()` and is still consumed by identifier-bearing legacy/v3 constructors. That policy rejects legal quoted PostgreSQL identifiers whose exact catalog content consists only of whitespace.
...
This is meaningful production progress but **not systemic GREEN**. Identifier-bearing call sites remain in `model.rs` and `representation_v3.rs`, including legacy schema/table/column/constraint/reference coordinates and qualified type/collation/operator-class/v3 coordinates. The remaining repair must route those identifier fields to the PostgreSQL-identifier admission rule while keeping rendered type text, reconstructed CHECK definitions, rendered expressions, extractor revision, option text, and other non-identifier metadata on field-specific nonblank policy. Globally weakening `validate_nonblank()` is not acceptable. Exact rationale and lineage are current in `docs/doctoring/postgresql-identifier-fidelity-systemic-integrity.md`.
