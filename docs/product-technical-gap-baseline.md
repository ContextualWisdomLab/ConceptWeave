...
- representation/index successor #46 source head before this documentation commit: `3c962108d09a8c02f3349ad79285ea8175ec1fa8`, OPEN Draft and mechanically mergeable.
...
- explicitly observed PRIMARY KEY/UNIQUE timing binds to same-relation/same-name supporting-index evidence and now requires uniqueness, PK/non-PK role, `indimmediate`, exact ordered simple key columns, non-partial shape and explicitly observed UNIQUE null-treatment coherence before immutable timing evidence is admitted.
...
Production repair `3c962108d09a8c02f3349ad79285ea8175ec1fa8` extends the same public aggregate admission seam without changing frozen v2 or original v3 digest identity. It now requires:
...
Static source review `5182395681` found the +25/-0 one-file repair causally aligned with the RED. `conindid` remains an adapter-local join coordinate, not governed semantic identity. No access-method inference was added: PostgreSQL 18 `WITHOUT OVERLAPS`/`pg_constraint.conperiod` is a separate temporal-key semantic and must be represented explicitly rather than normalized into ordinary B-tree uniqueness.
...
- workspace tests including frozen-v2 and retained v3 index/type/array/constraint contracts plus `constraint_timing_contract`, `constraint_backing_index_contract`, `constraint_backing_index_shape_contract`, `primary_key_invariants_contract`, `array_type_identity_contract`, `array_type_digest_contract`, `array_type_schema_contract` and `array_type_receipt_contract`;
...
- PostgreSQL Global Development Group. (2026). *PostgreSQL 18 documentation: pg_index — `indisunique`, `indisprimary`, `indimmediate`, `indnullsnotdistinct`, `indkey`, `indpred` and index state*.
...
| Source Observation | REPRESENTATION_V3_BACKING_INDEX_SHAPE_SOURCE_REPAIRED | `3c962108...` satisfies the shape/null-treatment RED at source level; exact-head Rust/Product acceptance remains mandatory. |
...
| PostgreSQL 18 temporal keys | EXPLICIT_NEXT_SEMANTIC_GAP | `WITHOUT OVERLAPS`/`conperiod` and PERIOD FK semantics are not yet first-class evidence; do not infer them. |
...
5. After representation/adapter prerequisites are GREEN, model PostgreSQL 18 temporal-key evidence (`conperiod`, `WITHOUT OVERLAPS`, PERIOD FK) explicitly rather than through DDL-text or access-method inference.
