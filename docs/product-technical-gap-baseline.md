...
- #46 `codex/pr6-v3-index-evidence`, OPEN Draft, is the active Source Observation writer. The PostgreSQL 18 first-class NOT NULL lineage includes aggregate admission, partition-parent validation, the `coninhcount` source-domain bound, corrected PRIMARY KEY completeness, and fail-closed rejection of partitioned-table `NO INHERIT` NOT NULL state. Exact-head native and hosted acceptance remain pending.
...
`.github#2170` preserves the bounded Required OpenCode `coverage-evidence` RCA source repair. Integration PRs #2176 and #2181 ordinary-merged the protected Noema advances into that branch. Current exact `9ae26db3bcac837162ffa452d1e704b6581ad0c5` is OPEN/Ready/mergeable at `SOURCE_REPAIRED / RESTACKED / ACCEPTANCE_PENDING`. Fresh Python Security `34773216587`, Semgrep `34773216564`, Runtime Quality `34773216613`, CodeQL `34773216625`, and Security Scan `34773216590` are queued/pending. Earlier-head execution/review evidence is historical only.

The protected CodeQL handler prerequisite `.github#2106` was likewise restacked through #2177 and #2182. Current exact `1ba96e4ddf6a800435651ec1c49acff533242fd9` is OPEN/Ready/mergeable. Fresh Semgrep `34773233354`, Python Security `34773233376`, Security Scan `34773233377`, CodeQL `34773233399`, and Runtime Quality `34773233339` are queued. Prior CodeQL chronology remains RCA history, not acceptance for this head.

`.github#2079` owns the Noema finding↔confirmed-probe contract. Integration PR #2179 ordinary-merged protected #2172/#2178 into the lane, producing exact `1438ba5d539b9b88b14bb6be4441f04a6e1a2c56`, OPEN/Draft/mergeable. The finding/probe source repair is preserved, but exact-current patch inspection still shows the four nested fixture methods `Response.__enter__`, `Response.__exit__`, `Response.read`, and `Opener.open` without docstrings in `test_call_llm_reports_allowed_location_truncation`. The predecessor Ready-generation measured 13/17 touched-callable docstrings; that execution result does not transfer, but the source gap does. #2079 is therefore `FINDING_PROBE_BINDING_SOURCE_REPAIRED / RESTACKED / DOCSTRING_RED`. Its fresh CodeQL `34773151824`, Python Security `34773151812`, Semgrep `34773151800`, and Security Scan `34773151840` are queued. The bounded causal fix remains four meaningful behavior-neutral docstrings; threshold weakening or synthetic reruns are invalid.
