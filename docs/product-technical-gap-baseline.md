...
- Procedural self-application #44: current branch successor of byte-transport source/test `d4a89af78153137f74eed1e6a6a2b64580b4d4a1` / `ce6b064abeb1db2184d7ecf6ea2d4756d500bbb6` and semantic-presence repair/test `7c607d8691fbe045dc419d21b2aff4777bad7043` / `d48e2731b390ae26bcba484cdafd087dc1d13b92`, OPEN Draft on #43. It adds inferred self-application profiles, strict checked-in artifact/provenance admission, an unpublished Rust semantic/topology projection, and a private strict JSON byte-transport recognizer. None of these promote artifact coordinates or parsed input to authority.
...
Eleven defects now have explicit RED/finding -> repair lineages on #44:
...
10. **An `&str`-only transport entry point could not own the wire boundary.** Review `5170467082` -> RED `a6c10bfd9219736c325c8195059c2e978e8f491a` -> production repair `d4a89af78153137f74eed1e6a6a2b64580b4d4a1` -> corrected boundary test successor `ce6b064abeb1db2184d7ecf6ea2d4756d500bbb6`. The owner seam now accepts borrowed `&[u8]`, rejects payloads over 2 MiB before UTF-8 validation, rejects malformed UTF-8 without replacement/lossy conversion, then runs the existing strict recognizer. The legacy `&str` convenience path delegates to the byte-first seam. Diagnostics remain fixed and non-echoing; no public/export or authority boundary widened.
11. **The Rust projection collapsed optional-field absence with schema-invalid present-empty `semantic_refs`.** Review `5171000125` -> RED/source contract `f9534b3a98341ff57505f0fc49529caf3b9f26ff` -> production repair `7c607d8691fbe045dc419d21b2aff4777bad7043` -> compatibility-test successor `d48e2731b390ae26bcba484cdafd087dc1d13b92`. `ProcedureNodeView.semantic_refs` is now `Option<&[ArtifactReferenceView]>`; `None` is genuine absence, `Some` is field presence, and `Some(&[])` fails `CollectionLimit` before artifact-reference validation. This preserves the Draft 2020-12 `minItems: 1` distinction without resolving artifact authenticity, ACL or capability.

The Rust modules remain deliberately unpublished. The byte-first transport seam validates bounded UTF-8 plus strict JSON grammar and duplicate-member determinism but does not deserialize or map the Draft 2020-12 procedural schema, authenticate expected scope, validate released artifact authenticity/ACL, evaluate semantics, approve a candidate, publish a release or authorize execution. Native Rust 1.98 fmt/strict Clippy/tests/rustdoc/release/owned coverage and exact-current Product acceptance are not established for the new byte-ingress or semantic-presence lineages. Do not promote source-shaped RED/repair lineage into exact-head GREEN.

The next procedural source gap remains lossless Rust transport-to-domain mapping of the canonical Draft 2020-12 shape. The projection can now represent `semantic_refs` absence versus presence correctly; the mapper must still deserialize without dropping schema-significant fields, reject unknown or otherwise schema-invalid fields before domain construction, preserve conditional `tool_contract_ref` semantics, and only then compare candidate scope against independently authenticated tenant/task/base/revision context. Released semantic/tool-contract authenticity, ACL/capability resolution, independent evaluation, retained rejected revisions, steward decision binding, immutable/CAS publication, revocation/rollback and released Noema projection remain separate later gates.
...
4. Restack dependent #5/#9/#43 and descendants normally and reset evidence. #44 preserves all eleven repair lineages but does not inherit acceptance from any predecessor head.
5. On #44, first obtain native Rust 1.98 RED/GREEN evidence for the byte-first strict transport and semantic-presence seams, then progress complete Draft 2020-12 transport-to-domain mapping and authenticated scope admission; do not repeat repaired semantic projection/transport findings or expand JS/local-Git checkers without a new valid finding.
