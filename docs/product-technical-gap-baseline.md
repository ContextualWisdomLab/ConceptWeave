...
Review `5202984522` found a composability error in the prior causal plan: a relation-`Var` successor cannot literally be layered on a successful v2 snapshot because v2 intentionally rejects every relation-Var-bearing tree. Weakening v2 would rewrite an issued proof. The new relation-`Var` family therefore branches from the exact expression-semantics predecessor while preserving both node-schema versions unchanged.
