...
Ten defects now have explicit RED/finding -> repair lineages on #44:
...
4. Restack dependent #5/#9/#43 and descendants normally and reset evidence. #44 preserves all ten repair lineages but does not inherit acceptance from any predecessor head.
5. On #44, first obtain native Rust 1.98 RED/GREEN evidence for the byte-first strict transport seam, then progress Draft 2020-12 transport-to-domain mapping and authenticated scope admission; do not repeat repaired semantic projection/transport findings or expand JS/local-Git checkers without a new valid finding.
