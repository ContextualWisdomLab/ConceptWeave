...
- representation/index successor #46 source head before this documentation commit: `c1abb9189d9f7c9539a65f87be654a6fe9ddbbc7`, OPEN Draft and mechanically mergeable.
...
Production repair `3f2ecba28fdd5742de5af1cac68227767baadbed` integrates an additive `conceptweave.postgres_schema_snapshot.v3.constraint_periods.v1` digest layer and one-time `with_observed_constraint_periods(...)` admission on the public v3 aggregate. It never reconstructs authoritative `conperiod` from `indisexclusion`, GiST, OIDs or rendered DDL; already represented index facts are used only as a contradiction check. `c1abb9189d9f7c9539a65f87be654a6fe9ddbbc7` keeps the new contract Clippy-style clean. This slice is source-repaired but remains acceptance-pending because no repository-pinned Rust 1.98 execution or hosted Product workflow exists on the moved head.
...
- workspace tests including frozen-v2 and retained v3 index/type/array/constraint contracts plus `constraint_timing_contract`, `constraint_backing_index_contract`, `constraint_backing_index_shape_contract`, `constraint_temporal_index_contract`, `constraint_period_contract`, `primary_key_invariants_contract`, `array_type_identity_contract`, `array_type_digest_contract`, `array_type_schema_contract` and `array_type_receipt_contract`;
...
- collect complete `pg_constraint` PK/UNIQUE timing, `conindid` support relationships and exact `conperiod` values for every represented PK/UNIQUE/FK before crossing the ACL;
...
- for PERIOD foreign keys, preserve the exact final period-column position, require at least one preceding equality-key column, and when the referenced relation is present in the same bounded snapshot require an explicitly observed `conperiod=true` PK/UNIQUE on the exact referenced columns;
...
- never infer temporal-key truth from reconstructed DDL or access-method conventions when direct catalog evidence exists;
...
- PostgreSQL Global Development Group. (2026). *PostgreSQL 18 documentation: CREATE TABLE — UNIQUE/PRIMARY KEY indexes, INCLUDE, NULLS NOT DISTINCT, WITHOUT OVERLAPS, PERIOD foreign keys, deferrability and constraint naming*.
...
| Source Observation | REPRESENTATION_V3_CONPERIOD_SOURCE_REPAIRED | `3f2ecba...` adds explicit domain-separated `conperiod` admission; `c1abb918...` is the code-current test cleanup; exact-head Rust/Product acceptance remains mandatory. |
...
| PostgreSQL 18 temporal keys | CONPERIOD_SOURCE_REPAIRED_ACCEPTANCE_PENDING | Explicit unobserved/false/true `conperiod` and PERIOD-FK coherence are now first-class source contracts; native/Product acceptance is still absent. |
