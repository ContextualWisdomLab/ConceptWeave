...
- Pending-source resolution #40: latest non-baseline predecessor `5f288daa49058a171a4dd7244cf85bc7a722102f`, OPEN Draft/mergeable on #9; the live PR ref is authoritative for this baseline-only successor. Source `610582a70f66503a694d4f1e72d34183a6652cdb` repairs unilateral stored item-identity drift, while coordinated-rewrite RED `a3cd9b0d68b4ecc28322860d04a23867412d611f` proves internal duplicated identity is not an external immutable-report binding. Status remains `REALITY_RED_TRUSTED_BINDING_PENDING`. #40 owns only the typed read-only exact-snapshot resolution aggregate; it has no Zotero write, semantic publication or approval authority.
- Full-text/write/recovery #39: freshly observed at `59b8761c62818e4bbd714d54354f47becd7bb944`, OPEN Draft on #38 `7678236ed3ec467e93b97bb2ad7ad26b3dc0e5b9`. This independent writer may advance normally; live PR metadata wins. Historical repository-count KPIs remain withdrawn.
...
`SourceResolutionReview` constructor admission rejects missing/blank report server identity, wrong server/library/item identity, duplicate/unknown decisions, missing decisions, blank reasons and missing retained inventory. Stored JSON must not regain exact-snapshot trusted status from internal agreement alone; restoration has to preserve the same immutable-report binding as constructor admission.
...
- Item-identity RED `6fa6c667e1f936f505ed6eae69e637b9e89dab71` mutates only the stored decision's `item_version`, `item_type`, or `parent_item_key`. Source repair `610582a70f66503a694d4f1e72d34183a6652cdb` adds `PendingSourceIdentity`, persists an `expected_source_identities` vector independently of steward decisions, requires its key sequence to equal canonical pending keys, and rejects unilateral version/type/parent drift. This is a causal source repair for that RED, subject to unchanged-head execution.
- Coordinated-rewrite RED `a3cd9b0d68b4ecc28322860d04a23867412d611f` then changes both `expected_source_identities[0].item_version` and `resolved_sources[0].item_version` together. Current deserialization compares only values carried inside the same mutable JSON artifact, so the coordinated rewrite can still regain the trusted `SourceResolutionReview` type. `docs/doctoring/zotero_source_resolution_item_identity.md` records the causal requirement: deserialize stored JSON as untrusted wire data and revalidate pending keys plus key/version/type/parent against the immutable `ClassificationReport` or an explicitly authenticated immutable report receipt before returning the trusted review type. Another mutable copy or self-declared digest in the same unauthenticated artifact is not a trust root.
- Style successor `d774c5b6382e71c78a32359cdef2c75c7f80a04d` changes only formatting in earlier completeness regressions. Historical docs successors remain evidence, not execution proof.
...
Current #40 status is **REALITY_RED_TRUSTED_BINDING_PENDING**, not GREEN. The latest pinned predecessor coverage run reports 173/182 branches (95.05%), 1,773/1,808 lines (98.06%), and 188/192 functions (97.92%); those metrics do not transfer to the changed RED/docs head and remaining gaps are recorded without exclusions. Keep Draft and keep valid review findings unresolved until trusted report-bound restoration is implemented and one unchanged exact successor completes the full Rust/coverage/hosted/independent-review gate.
...
| Pending-source resolution | REALITY_RED_TRUSTED_BINDING_PENDING | #40 rejects missing provider identity, nested server/library drift, duplicate/unsorted/blank stored decisions, blank stored rule revision, unilateral item-identity drift and simple decision truncation. Coordinated mutation of both stored expected identity and decision identity still bypasses constructor provenance because both are self-contained mutable JSON. RED `a3cd9b0...` requires report-bound or equivalently authenticated restoration before unchanged-head acceptance. |
