...
This refresh follows procedural self-application source/test head `7b536a62c428c8260e8712ba25714e55e03476a2`. RED head `49bfbc90d985c3cfaee8f12b44981741d6b41eb9` first wired an integration regression proving repository-owned JSON Schema could still collapse duplicate members before AJV; the minimal successors route procedural and semantic-candidate schema/fixture reads through the existing bounded strict JSON boundary. The commit containing this file is a documentation-only successor, so source-head local evidence does not automatically become exact-current whole-head evidence.
...
Owner repair must preserve authenticated repository/PR/head/base/required-run/language/job/SARIF/artifact evidence, boundedly converge the complete terminal job set, and perform one run-wide wake before producing terminal receipt. Fresh consumer canaries observed on 2026-09-10 continue to reproduce receiver-before-dispatch ordering on unchanged exact heads, so this remains a central owner defect rather than a ConceptWeave leaf workaround. Old-base hosted evidence does not transfer after ancestry repair.
...
- Procedural self-application #44: source/test head `7b536a62c428c8260e8712ba25714e55e03476a2`, OPEN Draft on #43. It adds inferred self-application profiles, a Rust topology/evidence-membership candidate, local-Git source-provenance verification, strict duplicate-key admission for profiles plus repository-owned schemas/fixtures, regular-file path-mode binding for provenance sources, bounded non-echoing duplicate-key diagnostics, and a provenance receipt that no longer overclaims source authentication.
...
Seven artifact-admission defects now have explicit RED -> repair evidence on #44:
...
5. **Fail-closed duplicate detection could still amplify untrusted content into diagnostics.** Exact-head review `5164233960` reproduced a 1,800,011-byte valid duplicate-member input whose ~900 KiB member name became a 900,029-byte exception message. RED-only commit `4fac2eb62f5484c20bce44c5abfc57b93592a596` adds the bounded-diagnostic regression. GREEN source `10799a65325319c6ff337814b23d9d7a48d82fcd` replaces raw member-name echo with a 16-hex SHA-256 fingerprint plus exact UTF-8 byte count. Local Node 22.16 strict-JSON slice is **7 tests / 7 passed**, including decoded-key equivalence, malformed/trailing input, depth/size bounds and the 900,000-byte duplicate-key witness; the repaired diagnostic remains <=128 bytes for the production label used by the checker. No semantic value or source text is emitted in the duplicate-key error.
6. **Local repository integrity was exposed as generic source authentication.** Exact-head review `5164920106` identified `source_authentication_established: true` as broader than the verifier's actual evidence: ancestor `commit:path`, regular Git blob identity and exact SHA-256 inside the checked-out repository. RED-only test head `c476ce244a6e6774a8b1d12c7b38613deaada2db` requires a narrowly named local-Git receipt and rejects the old broad field. GREEN source/test head `abb8d7f98973d6ba77799b9bbc69d82353f48454` returns `local_git_provenance_established: true` and omits `source_authentication_established`. A synthetic one-source Git repository executed under Node 22.16 confirmed exact commit/path/blob/SHA-256 verification and the narrowed result object. This does not establish signer identity, external evidence authorization, factual truth or governance authority.
7. **The validator definitions themselves still admitted ambiguous JSON.** Exact-head review `5165560999` found that `check_procedural_contracts.mjs` and `check_semantic_candidate_contracts.mjs` continued to parse repository-owned schemas and fixtures with ordinary `JSON.parse`, allowing duplicate members to collapse before AJV compiled the effective contract. Test commit `dfe1f74aa76981f533ea071037f7fc72134e309d` adds the schema-loader witness and RED head `49bfbc90d985c3cfaee8f12b44981741d6b41eb9` wires it into the package contract suite. A Node 22.16 execution against the predecessor loader produced `ERR_ASSERTION: Missing expected exception`. Repair `7c15ec12740854261cb9d931e804304c53031477` routes procedural schemas and fixture JSON through `parseJsonRejectDuplicateKeys`; source/test successor `7b536a62c428c8260e8712ba25714e55e03476a2` applies the same rule to semantic-candidate schema/fixtures. The focused integration witness is 1/1 GREEN with a deterministic AJV test double after the repair, and both modified checker modules pass Node syntax validation. This local seam proves duplicate-member rejection occurs before AJV; it is not a substitute for the locked real-AJV whole-suite or Rust/Product acceptance.
...
1. Central `.github` owners adopt protected `cb0872c...` non-force, repair terminal settlement with the stable #35 and other fresh consumer REDs, reconcile #2051/#2056 valid deltas, obtain exact-current GREEN/review and integrate normally.
...
4. Restack dependent #5/#9/#43 and descendants normally and reset evidence. #44 retains the seven artifact-admission RED/repair lineages.
