...
- Pending-source resolution #40: latest non-baseline predecessor `40a908c14af52a0470e1068772f5c4c4f3eff7d0`, OPEN Draft/mergeable on #9; the live PR ref is authoritative for this baseline-only successor. Stored-completeness RED `6b28fc38afaae455a70901c57b28116b0b88510d` is repaired, but item-identity RED `6fa6c667e1f936f505ed6eae69e637b9e89dab71` remains `REALITY_RED_SOURCE_FIX_PENDING`. #40 owns only the typed read-only exact-snapshot resolution aggregate; it has no Zotero write, semantic publication or approval authority.
...
`SourceResolutionReview` constructor admission rejects missing/blank report server identity, wrong server/library/item identity, duplicate/unknown decisions, missing decisions, blank reasons and missing retained inventory. Stored JSON must retain independent completeness and item-identity evidence and revalidate the same boundary rather than validating only whichever decision payload happens to remain in the artifact.
...
- Item-identity RED `6fa6c667e1f936f505ed6eae69e637b9e89dab71` starts from a constructor-valid one-source review and mutates only the stored decision's `item_version`, `item_type`, or `parent_item_key`. Current deserialization has no independent expected values for those coordinates, so the altered payload can regain typed status even though constructor admission would reject it against the report. `docs/doctoring/zotero_source_resolution_item_identity.md` records the least-widening repair: persist constructor-bound expected key/version/type/parent coordinates separately from steward decisions and require exact equality during restoration. Do not derive expected identity from the mutable decision array itself.
- Style successor `d774c5b6382e71c78a32359cdef2c75c7f80a04d` changes only formatting in the earlier completeness regressions. Historical docs successors remain evidence, not execution proof.
...
| Pending-source resolution | REALITY_RED_SOURCE_FIX_PENDING | #40 rejects missing provider identity, nested server/library drift, duplicate/unsorted/blank stored decisions, blank stored rule revision and truncated stored decision sets, but restored JSON still lacks independent expected item version/type/parent evidence. RED `6fa6c667...` must be causally repaired before unchanged-head acceptance. |
