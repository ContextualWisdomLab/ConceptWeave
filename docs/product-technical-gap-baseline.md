...
- Pending-source resolution #40: source-repair head `6dbc013d81a4cff1e46fbe43f4d920d968c425c0`, OPEN Draft/mergeable on #9. It owns only the typed read-only exact-snapshot resolution aggregate; it has no Zotero write, semantic publication or approval authority.
...
- Reality RED `d6867f916b00948586a53129da9b0cf17cf2bbad` mutates only a constructor-valid stored review's `rule_revision` to whitespace. The prior custom deserializer admitted that artifact, erasing the classifier revision needed to replay which deterministic rule set produced the pending inventory.
...
| Pending-source resolution | SOURCE_TEST_REPAIRED_PENDING_CI | #40 rejects missing provider identity, nested server/library drift, duplicate/unsorted/blank stored decisions and blank stored rule revision. Full unchanged-head acceptance remains outstanding. |
