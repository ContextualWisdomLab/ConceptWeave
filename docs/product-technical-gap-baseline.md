...
- protected `ContextualWisdomLab/.github/main@eb993211e01d759670328789eb4d8f2b8c2ec024`;
- `.github#2040@9f435af94de5e02897b8d132105dea5f03b33547`, OPEN / Draft / non-mergeable;
- compare status `diverged`, 158 ahead / 238 behind, merge base `fb17ef556f94f673234aa557254ae52779e9a7b0`;
...
Fresh current-base review `5239422933` found a separate inherited private-consumer permissions gap on #2040 predecessor `b1542f0e21d6709d273af04b80f595a7d0fcdaf7`. `analyze-head` and `dispatch-current-head` read live target PR metadata and target commit statuses, but the predecessor job tokens omitted `pull-requests: read` and `statuses: read`; protected main already carried both grants after a live private-consumer HTTP 403. Structural RED `2a7e1252a57fd2aee934bb9c531b4cbecfc49f66` pins those job-scoped grants and forbids `actions: write`. Minimal production repair `9f435af94de5e02897b8d132105dea5f03b33547` adds only the four missing read-grant lines, preserving all v2 producer, base-bound receipt/direct-evidence, rerun/job-set, OIDC/app-token and no-source-neutral-restamp behavior. Exact-head owner checkpoint review is `5239461318`.

The six protected commits from `3449d0020ffac86315ecccfb9d5a1dd3bf421834` through `eb993211e01d759670328789eb4d8f2b8c2ec024` change queue-health enrollment/doctoring/test surfaces only and are treated as intervening protected history rather than a race. The remaining #2040 overlap still requires path-wise semantic reconciliation, especially CodeQL producer/handler, review scheduler, ADR/CHANGELOG/product-gap and their tests; whole-tree ours/theirs replacement is not accepted.

Exact `.github#2040@9f435af9...` currently has no PR-triggered workflow run generation. Combined commit statuses show Devin Review and CodeRabbit success only; these do not substitute for terminal required-workflow evidence. No qualifying independent current-head `APPROVED` review exists.
