...
The client computes a separate domain-separated digest over the complete release manifest, including release identity, versions, state, declared artifact digest, provenance, and concept IDs. The canonical encoding is specified in `contracts/semantic-release-manifest-digest-v1.md`. A matching value from the release being checked cannot install its own trust pin: protected Governance & Publication distribution must supply the pinned digest independently. This client contract closes flag-only admission; it does not itself authenticate a steward or distribute protected pins.

Governance review now accepts only a complete validated alignment with at least one included concept and asks an injected steward-authority adapter to authorize its exact proposal, source digest, alignment digest, steward, and rationale. A denial cannot produce a reviewed alignment. Publication constructs canonical immutable bytes itself, binds the authority's audit receipt, and derives the release artifact digest and client manifest pin from those bytes. `contracts/governed-semantic-artifact-v2.md` specifies the current byte contract, including exact relational source facts retained for review and client interpretation. A production authority adapter, protected storage deployment, and independent pin distribution remain required before a real steward decision or buyer release can be claimed.

The local publication adapter now issues a release only after an atomic, durable first-writer-wins file claim on its release ID. It rejects an identical retry and a different artifact under the same ID. Readback requires an independently pinned client release and verifies both the complete stored manifest fields and exact artifact bytes. `contracts/immutable-release-record-v1.md` specifies the record and crash boundary. A protected production root, backup/recovery policy, production steward authentication, and independent pin distribution remain necessary for buyer release operation.
