...
Initial finding review `5254405649` pins the original missing `pg_init_privs` fact at exact pre-finding #46 head `71c86b196ab1fa2e66143d8f0f708a2606ace724`. Source-order integrity finding review `5255912747` pins the later normalization defect at exact pre-finding head `4967f12cb90ac41ec42263bfed47ba6bcbb49735`.
...
ACL entry order is source and recovery identity. It is preserved exactly as observed from `pg_init_privs.initprivs`. Duplicate ACL entries still fail closed, but duplicate detection is order-independent and cannot normalize the sequence as a side effect. The sorted/deduplicated dangling-role OID collections used for remediation are derived diagnostic sets only; they do not replace or reorder the source ACL.
...
- Source-order production repair: `966e2136c15aa049f72acfebf2c1a478d09c66ef` removes `grants.sort()`, hashes and retains grants in source order, and replaces adjacency-based duplicate detection with an order-independent set check.
...
5. ACL entry order is identity-bearing and remains exactly source-observed; PUBLIC versus a named role, unresolved role identity, exact grantor, and grant option are also identity-bearing.
6. Duplicate ACL entries, blank resolved role identities, missing/extra/duplicate coordinates, binding drift, zero key positions, unknown receipt locations, and provenance-location collisions fail closed without reordering the source ACL.
...
The differential must demonstrate at least one same-generation case in which the current function ACL is held constant while the initial privilege baseline differs, and must verify row absence, `privtype`, complete initial EXECUTE ACL resolution, immutable raw-root lineage, downstream transform-object digest propagation, and source-array order. At least one fixture must demonstrate that swapping the same ACL entries changes ConceptWeave material identity and readback order; when a live PostgreSQL baseline contains grant-option dependency ordering, that observed order must survive intact. Synthetic data remains suitable for unit contracts only; acceptance needs live PostgreSQL catalog evidence.
...
PostgreSQL Global Development Group. (2026f). *PostgreSQL 18 documentation: pg_dump*. https://www.postgresql.org/docs/18/app-pgdump.html
