...
At PostgreSQL `REL_18_STABLE` commit `6567f3f1f21353b5b0e9538d058998ab76c5a37c`, `src/backend/utils/adt/pg_locale_libc.c::get_collation_actual_version_libc()` leaves the actual version NULL when the libc collation locale is case-insensitively `C`, begins with `C.`, or is `POSIX`. `src/backend/commands/dbcommands.c::AlterDatabaseRefreshColl()` obtains `pg_database.datcollate` for `datlocprovider = 'c'` and calls the same provider dispatcher, so the database-default actual-version path has the same C-family/POSIX invariant as material libc collations.
...
- PostgreSQL authority: `postgres/postgres@6567f3f1f21353b5b0e9538d058998ab76c5a37c`, `src/backend/utils/adt/pg_locale_libc.c::get_collation_actual_version_libc`, `src/backend/commands/dbcommands.c::AlterDatabaseRefreshColl`
