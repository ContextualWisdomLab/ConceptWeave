...
PostgreSQL also retains one deliberate escape path: a superuser may create a `SQL_ASCII` database even when the locale reports another encoding. The representation cannot discard that source-reachable catalog state merely because ordinary callers would be rejected.
...
Review `5213788571` identified the gap. Source contract commit `9e9379359406787d0e9cf7b964e7800f63055496` adds `database_default_libc_c_utf8_database_encoding_contract.rs` and requires C-UTF8 in either `datcollate` or `datctype` to fail on LATIN1 while retaining UTF8 and C/POSIX controls.

The predecessor implementation returned `true` for every libc provider in `validate_database_encoding()`, so the LATIN1 witnesses are source-level RED against the reviewed predecessor.

## Repair and retained exception

Initial production commit `336c42aaadeb729a65f2343bf3501826c9478343` changed only the libc arm of `validate_database_encoding()` and added `is_c_utf8_libc_locale()`. Follow-up source review found that the first version was too narrow in the opposite direction: PostgreSQL 18 `check_encoding_locale_matches()` explicitly accepts an otherwise mismatched locale when the target database encoding is `SQL_ASCII` and creation is authorized as superuser. Rejecting `C.UTF-8` + `SQL_ASCII` would therefore create a false negative.

Ordinary-forward contract correction `68d4a6064822999ee4b1544c965476431fb8bd1f` adds SQL_ASCII as a positive control. Production correction `d690f3f94f71cae8a6e137693383ef6271d0614f` now requires a C-UTF8 libc database locale to use either PostgreSQL UTF8 encoding ID `6` or the source-reachable SQL_ASCII encoding ID `0` path. LATIN1 remains rejected. `C`/`POSIX`, built-in, ICU, version evidence, provider-d coherence, predecessor digests, and canonical database-default digest encoding are unchanged.
...
- Requiring every libc database default to be UTF8 was rejected because PostgreSQL supports libc databases in other compatible encodings and retains the explicit superuser SQL_ASCII exception.
...
The live differential must include UTF8 + C-UTF8 acceptance, LATIN1 + C-UTF8 rejection, the authorized SQL_ASCII + C-UTF8 path, and C/POSIX cross-encoding controls. Exact-head Rust execution is still required after this ordinary-forward source repair; earlier execution evidence does not transfer after the head moves.
