...
A local/unattached child index remains legal even when its owning table is a partition. Attachment is asserted only when the index evidence itself carries the exact direct parent.
...
This chronology is intentional evidence: the implementation text existed before the RED commit, but it was not reachable through the production crate API. The executable external contract preceded the commit that made the production path active. No claim is made that Rust RED or GREEN was executed on this host.
...
7. the parent graph is acyclic before digesting;
8. input order does not affect canonical identity; topology does.
