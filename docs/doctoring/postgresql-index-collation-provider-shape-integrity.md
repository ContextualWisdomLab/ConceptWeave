...
`REL_18_STABLE` `CollationCreate()` asserts the ordinary created-row shape: libc requires both `collcollate` and `collctype` and forbids `colllocale`; non-libc created rows forbid the two libc fields and require `colllocale`. `CREATE COLLATION` restricts nondeterministic collations and `RULES` to ICU and restricts the built-in provider locale to `C`, `C.UTF-8`, or `PG_UNICODE_FAST`.

The bootstrap catalog has one deliberate exception to the ordinary non-libc constructor shape: `pg_catalog.default` uses provider `d`, `collencoding = -1`, default deterministic mode, and no locale/rules fields. Its effective behavior is already represented separately by `IndexEffectiveCollationDefinitionSnapshot` over `pg_database`.
...
The focused contract pins invalid libc/non-libc field mixtures, non-ICU nondeterminism/rules, the PostgreSQL 18 built-in locale set, the valid ICU nondeterministic/rules case, and the bootstrap database-default shape. Production rejects violations with `index_collation_definition_provider_shape` before the observation can be hashed.
...
After representation acceptance, the concrete adapter must obtain the exact used `pg_collation` rows and actual provider versions in the same bounded read-only catalog observation and run live differential cases for libc, built-in, ICU, and the `pg_catalog.default` delegation path. The adjacent `pg_database` default-definition constructor must be reviewed independently for the same provider/field-shape class rather than assuming this `pg_collation` repair proves that separate catalog contract.
