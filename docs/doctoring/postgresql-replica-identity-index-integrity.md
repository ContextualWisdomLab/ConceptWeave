...
PostgreSQL 18 `pg_index` defines `indisreplident` as true when an index has been chosen as replica identity with `ALTER TABLE ... REPLICA IDENTITY USING INDEX`. The same catalog distinguishes `indnkeyatts` key positions from later non-key included positions in `indkey`; key columns precede included columns, and per-key collation/operator-class/option arrays have `indnkeyatts` entries:
...
- PostgreSQL Global Development Group. (2026). *PostgreSQL 18 documentation: CREATE TABLE — INCLUDE*. https://www.postgresql.org/docs/18/sql-createtable.html
- PostgreSQL Global Development Group. (2026). *PostgreSQL source: relcache.c — replica identity attribute bitmap*. https://github.com/postgres/postgres/blob/REL_18_STABLE/src/backend/utils/cache/relcache.c
...
- every key position of a replica-identity index must resolve to an owning relation column observed as `NOT NULL`; checking only the first key is insufficient for a multi-column identity.
- nullable `INCLUDE` payload remains outside replica-identity key material and therefore must not be rejected by the identity-key NOT NULL rule.
- a unique, non-partial, immediate, column-key-only table index over `NOT NULL` key columns remains admissible.
