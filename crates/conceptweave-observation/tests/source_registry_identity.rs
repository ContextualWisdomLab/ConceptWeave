...
fn snapshot_with_source(source_connection_key: &str) -> Result<PostgresSchemaSnapshot, ObservationError> {
...
fn snapshot_accepts_a_bounded_multiword_snake_case_registry_identity() {
    let snapshot = snapshot_with_source("grc_readonly_connection")
        .expect("the immutable snapshot accepts the same opaque registry identity as the source port");

    assert_eq!(snapshot.source_connection_key(), "grc_readonly_connection");
    let table_location = conceptweave_observation::ObservationLocation::table("public", "event_record")
