...
impl PostgresSchemaSnapshot {
    ...
    /// source connection reference must be a registry-resolved capability issued by the Source
    /// Observation port; a caller cannot substitute raw connection text when constructing the
    /// immutable aggregate. The observation time must be an RFC 3339-style timestamp with an
    /// explicit UTC `Z` designator.
    pub fn new(
        ...
        let source_connection_key = source_connection.source_connection_key().to_owned();
