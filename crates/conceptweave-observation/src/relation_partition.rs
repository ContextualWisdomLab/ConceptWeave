//! Source-authoritative PostgreSQL 18 declarative-partition membership evidence.
//!
//! `pg_class.relkind` does not identify whether an otherwise ordinary or partitioned table is
//! itself a declarative partition. PostgreSQL records that fact separately in `pg_class.relispartition`
//! and records the direct parent edge in `pg_inherits`. This module preserves that source truth in a
//! domain-separated evidence family without changing the frozen base-v3 digest contract.
...
/// coordinate to exact schema/relation text before entering this contract. Parent relation kind is
/// validated against the bounded base snapshot and therefore is not caller-supplied twice.
...
impl PartitionParentRelationCoordinate {
    ...
    pub fn new(
        ...
        crate::model::validate_nonblank(&schema_name, "relation_partition_parent_schema_name")?;
...
///
/// Generic table inheritance is deliberately outside this value object. A `non_partition`
/// observation means only that `relispartition=false`; it does not assert that `pg_inherits` has no
/// generic-inheritance edge. Positive partition membership carries exactly one direct parent because
/// PostgreSQL declarative partitions cannot have another parent in either the declarative or generic
...
impl RelationPartitionObservation {
    ...
    ///
    /// `detach_pending` must be supplied from `pg_inherits.inhdetachpending`. A pending detach is a
    /// transitional topology rather than stable publishable membership, so it fails closed instead
    /// of being normalized to an attached partition.
    pub fn partition(
        schema_name: impl Into<String>,
        relation_name: impl Into<String>,
        relation_kind: RelationKind,
    ...
    fn new(
        ...
        let schema_name = schema_name.into();
        let relation_name = relation_name.into();
        crate::model::validate_nonblank(&schema_name, "relation_partition_schema_name")?;
        crate::model::validate_nonblank(&relation_name, "relation_partition_relation_name")?;
            ...
            )
        {
            return Err(ObservationError::InvalidObservationField {
                field: "relation_partition_relation_kind",
            });
        }
        if detach_pending {
            return Err(ObservationError::InvalidObservationField {
                field: "relation_partition_detach_pending",
            });
        }
        if is_partition != parent_relation.is_some() {
            return Err(ObservationError::InvalidObservationField {
                field: "relation_partition_parent_presence",
            });
        }
        if parent_relation.as_ref().is_some_and(|parent| {
            ...
            return Err(ObservationError::InvalidObservationField {
                field: "relation_partition_parent",
            });
        }
    ...
    /// Returns the exact observed `pg_class.relkind` already bound by the base snapshot.
    #[must_use]
    ...
    ///
    /// Admitted immutable observations are always false because transitional detach state fails
    /// closed at construction. The accessor remains explicit so callers never need to invent a
    /// default for the catalog column.
    #[must_use]
...
impl RelationPartitionLocation {
    ...
    pub fn new(
        ...
        let schema_name = schema_name.into();
        let relation_name = relation_name.into();
        crate::model::validate_nonblank(&schema_name, "relation_partition_location_schema_name")?;
        crate::model::validate_nonblank(
            &relation_name,
    ...
    pub fn canonical_location(&self) -> String {
        format!(
            ...
            crate::model::escape_json_pointer_token(&self.schema_name),
            self.relation_kind.token(),
            crate::model::escape_json_pointer_token(&self.relation_name)
        )
    }
}

/// Immutable receipt binding exact partition-membership evidence to its governed digest/provenance.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl RelationPartitionSourceReceipt {
    ...
    /// Returns the immutable connection-policy revision used by the base snapshot.
    #[must_use]
    pub fn connection_policy_binding(&self) -> &str {
        &self.connection_policy_binding
    }

    /// Returns the relation-partition family's governed source digest.
    #[must_use]
    pub fn source_digest(&self) -> &str {
        &self.source_digest
    }

    /// Returns the exact extractor revision inherited from the bounded base snapshot.
    #[must_use]
    pub fn extractor_revision(&self) -> &str {
        &self.extractor_revision
    }

    /// Returns the exact canonical UTC observation time inherited from the base snapshot.
    #[must_use]
...
/// Complete immutable declarative-partition evidence layered over one exact v3 snapshot.
///
/// Every bounded relation receives one explicit `relispartition` observation, so observed false is
/// distinct from family absence. Positive membership must resolve to an observed partitioned-table
/// parent, the parent graph must be acyclic, detach-pending state is rejected, and any independently
/// observed NOT NULL partition witness must agree with the same direct relation edge. The digest is
/// domain-separated and frames the predecessor snapshot digest, preserving the frozen base-v3
/// meaning while making partition membership material successor identity.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl RelationPartitionSnapshot {
    /// Creates complete relation-partition evidence over one exact bounded base snapshot.
    pub fn new(
    ...
    /// Returns the immutable connection-policy revision inherited from the base snapshot.
    #[must_use]
    pub fn connection_policy_binding(&self) -> &str {
        &self.connection_policy_binding
    }
    ...
    /// Returns the exact extractor revision inherited from the base snapshot.
    #[must_use]
    pub fn extractor_revision(&self) -> &str {
        &self.extractor_revision
    }

    /// Returns the exact UTC observation time inherited from the base snapshot.
    #[must_use]
    pub fn observed_at_utc(&self) -> &str {
        &self.observed_at_utc
    }
...
fn canonicalize_relation_partitions(
    ...
    if observed_coordinates.len() != observations.len() {
        return Err(ObservationError::InvalidObservationField {
            field: "relation_partition_coordinate",
        });
    }
    if observed_coordinates != expected_coordinates {
        return Err(ObservationError::InvalidObservationField {
            field: "relation_partition_completeness",
        });
    }
    ...
    for observation in &observations {
        if let Some(parent) = observation.parent_relation() {
            let Some(parent_relation) = base_snapshot.relations().iter().find(|relation| {
                ...
                return Err(ObservationError::InvalidObservationField {
                    field: "relation_partition_parent_coordinate",
                });
            };
            if parent_relation.kind() != RelationKind::PartitionedTable {
                return Err(ObservationError::InvalidObservationField {
                    field: "relation_partition_parent_kind",
                });
            }
        }
    }
    ...
    for root in by_coordinate.keys() {
        ...
        loop {
            if !path.insert(current.clone()) {
                return Err(ObservationError::InvalidObservationField {
                    field: "relation_partition_cycle",
                });
            }
...
fn validate_not_null_partition_witnesses(
    ...
    for not_null in not_null_constraints {
        ...
        if !agrees {
            return Err(ObservationError::InvalidObservationField {
                field: "relation_partition_not_null_parent",
            });
        }
    }
    Ok(())
