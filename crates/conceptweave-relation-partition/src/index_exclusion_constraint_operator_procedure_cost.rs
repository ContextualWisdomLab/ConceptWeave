//! PostgreSQL ordinary exclusion-constraint implementation-function planner-cost evidence.
//!
//! `pg_proc.procost` is an independent `float4` planner estimate measured in units of
//! `cpu_operator_cost`. PostgreSQL allows the value to change without changing the target function's
//! call signature or the already-governed execution, security, definition, ACL, and planner-support
//! facts. This observational successor therefore preserves the exact positive finite float4 value
//! without imposing a product-specific cost threshold.
...
use super::{
    ...
    IndexExclusionConstraintOperatorProcedurePlannerSupportSnapshot, QualifiedOperatorSignature,
    QualifiedProcedureSignature,
};

const INDEX_EXCLUSION_CONSTRAINT_OPERATOR_PROCEDURE_COST_DIGEST_DOMAIN_V1: &[u8] =
    b"conceptweave.postgres_schema_snapshot.v3.relation_partition.index_partition.exclusion_constraint.operator.procedure.cost.v1";
const SHA256_DIGEST_PREFIX: &str = "sha256:";

/// Exact same-row `pg_proc.procost` observation for one governed ordinary-EXCLUDE function.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureCostObservation {
...
impl IndexExclusionConstraintOperatorProcedureCostObservation {
    ...
    /// Returns the exact positive finite `pg_proc.procost` value captured as PostgreSQL `float4`.
    #[must_use]
    pub fn execution_cost(&self) -> f32 {
        f32::from_bits(self.execution_cost_bits)
    }

    /// Returns the collision-safe evidence location for this planner-cost fact.
    #[must_use]
    pub fn canonical_location(&self) -> String {
        procedure_cost_location(&self.coordinate, self.key_position)
    }
}

/// Immutable provenance receipt for one exact implementation-function planner-cost observation.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureCostSourceReceipt {
    source_id: String,
    connection_policy_binding: String,
    source_digest: String,
    extractor_revision: String,
    observed_at_utc: String,
    location: IndexExclusionConstraintOperatorProcedureCostObservation,
}

impl IndexExclusionConstraintOperatorProcedureCostSourceReceipt {
    ...
    /// Returns the owner-computed planner-cost successor digest.
    #[must_use]
    ...
    /// Returns the exact validated planner-cost observation.
    #[must_use]
    pub const fn location(&self) -> &IndexExclusionConstraintOperatorProcedureCostObservation {
        &self.location
    }
}

/// Complete `pg_proc.procost` evidence over one exact planner-support predecessor.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureCostSnapshot {
    source_connection_key: String,
    connection_policy_binding: String,
    snapshot_digest: String,
    extractor_revision: String,
    observed_at_utc: String,
    observations: Vec<IndexExclusionConstraintOperatorProcedureCostObservation>,
}

impl IndexExclusionConstraintOperatorProcedureCostSnapshot {
    /// Creates complete planner-cost evidence over one exact planner-support predecessor.
    pub fn new(
        planner_support_snapshot: &IndexExclusionConstraintOperatorProcedurePlannerSupportSnapshot,
        mut observations: Vec<IndexExclusionConstraintOperatorProcedureCostObservation>,
    ) -> Result<Self, ObservationError> {
        ...
        let snapshot_digest = compute_procedure_cost_digest(
    ...
    /// Returns the domain-separated implementation-function planner-cost successor digest.
    #[must_use]
    ...
    /// Returns complete planner-cost observations in deterministic constraint/key order.
    #[must_use]
    pub fn observations(&self) -> &[IndexExclusionConstraintOperatorProcedureCostObservation] {
        &self.observations
    }

    /// Issues exact provenance for one observed implementation-function planner-cost identity.
    pub fn source_receipt(
        &self,
        coordinate: IndexExclusionConstraintCoordinate,
        key_position: u32,
    ) -> Result<IndexExclusionConstraintOperatorProcedureCostSourceReceipt, ObservationError> {
        let observation = self
            ...
            .ok_or_else(|| ObservationError::UnknownObservationLocation {
                location: procedure_cost_location(&coordinate, key_position),
            })?;
        Ok(IndexExclusionConstraintOperatorProcedureCostSourceReceipt {
...
fn compute_procedure_cost_digest(
    predecessor_digest: &str,
    observations: &[IndexExclusionConstraintOperatorProcedureCostObservation],
) -> String {
    let mut hasher = Sha256::new();
    hasher.update(INDEX_EXCLUSION_CONSTRAINT_OPERATOR_PROCEDURE_COST_DIGEST_DOMAIN_V1);
...
fn procedure_cost_location(
    coordinate: &IndexExclusionConstraintCoordinate,
    key_position: u32,
) -> String {
    format!(
        "{}/exclusion-operators/{key_position}/procedure-cost",
