//! PostgreSQL ordinary exclusion-constraint operator implementation-function evidence.
//!
//! PostgreSQL stores the implementation function of each operator in `pg_operator.oprcode`, while
//! the exclusion-index runtime separately carries the underlying function OID for every exclusion
//! operator. Matching operand types do not prove those two independently sourced facts identify the
//! same function. This successor preserves the resolved `oprcode -> pg_proc` signature for every
//! exact ordinary-EXCLUDE operator position and rejects disagreement with the already-governed
//! backing exclusion procedure without rewriting any predecessor digest domain.
...
const INDEX_EXCLUSION_CONSTRAINT_OPERATOR_PROCEDURE_DIGEST_DOMAIN_V1: &[u8] =
    b"conceptweave.postgres_schema_snapshot.v3.relation_partition.index_partition.exclusion_constraint.operator.procedure.v1";
const SHA256_DIGEST_PREFIX: &str = "sha256:";

/// Exact independently resolved `pg_operator.oprcode` evidence for one ordinary EXCLUDE key.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureObservation {
...
impl IndexExclusionConstraintOperatorProcedureObservation {
    /// Records one exact governed operator and the procedure independently resolved from `oprcode`.
    pub fn new(
        coordinate: IndexExclusionConstraintCoordinate,
        key_position: u32,
        operator: QualifiedOperatorSignature,
    ...
    /// Returns the governed operator signature repeated at this exact key position.
    #[must_use]
    pub const fn operator(&self) -> &QualifiedOperatorSignature {
        &self.operator
    }

    /// Returns the stable procedure signature independently resolved from `pg_operator.oprcode`.
    #[must_use]
    pub const fn procedure(&self) -> &QualifiedProcedureSignature {
        &self.procedure
    }

    /// Returns the collision-safe evidence location for this operator-procedure edge.
    #[must_use]
    pub fn canonical_location(&self) -> String {
        procedure_location(&self.coordinate, self.key_position)
    }
}

/// Immutable provenance receipt for one exact exclusion operator-procedure observation.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureSourceReceipt {
    source_id: String,
    connection_policy_binding: String,
    source_digest: String,
    extractor_revision: String,
    observed_at_utc: String,
    location: IndexExclusionConstraintOperatorProcedureObservation,
}

impl IndexExclusionConstraintOperatorProcedureSourceReceipt {
    ...
    /// Returns the owner-computed operator-procedure successor digest.
    #[must_use]
    ...
    /// Returns the exact validated operator-procedure observation.
    #[must_use]
    pub const fn location(&self) -> &IndexExclusionConstraintOperatorProcedureObservation {
        &self.location
    }
}

/// Complete independently resolved operator implementation evidence over one ordinary EXCLUDE stack.
///
/// Every governed `pg_constraint.conexclop` position receives exactly one `pg_operator.oprcode`
/// observation. The repeated operator must equal the exact predecessor position and the resolved
/// procedure must equal the backing exclusion procedure retained from that predecessor's independently
/// rebound index semantics. A same-typed but different function therefore fails closed. OIDs remain
/// adapter-local join coordinates; only stable resolved operator/procedure signatures enter the digest.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureSnapshot {
    source_connection_key: String,
    connection_policy_binding: String,
    snapshot_digest: String,
    extractor_revision: String,
    observed_at_utc: String,
    observations: Vec<IndexExclusionConstraintOperatorProcedureObservation>,
}

impl IndexExclusionConstraintOperatorProcedureSnapshot {
    /// Creates complete operator implementation-function evidence over one exact operator predecessor.
    pub fn new(
        operator_snapshot: &IndexExclusionConstraintOperatorSnapshot,
        mut observations: Vec<IndexExclusionConstraintOperatorProcedureObservation>,
    ) -> Result<Self, ObservationError> {
        ...
        if observed.len() != observations.len() {
            return Err(invalid(
                "index_exclusion_constraint_operator_procedure_coordinate",
            ));
        }
        if observed != expected {
            return Err(invalid(
                "index_exclusion_constraint_operator_procedure_completeness",
            ));
        }
    ...
    /// Returns the domain-separated operator-procedure successor digest.
    #[must_use]
    ...
    /// Returns complete procedure observations in deterministic constraint/key order.
    #[must_use]
    pub fn observations(&self) -> &[IndexExclusionConstraintOperatorProcedureObservation] {
        &self.observations
    }
    ...
    pub fn source_receipt(
        ...
        coordinate: IndexExclusionConstraintCoordinate,
        key_position: u32,
    ) -> Result<IndexExclusionConstraintOperatorProcedureSourceReceipt, ObservationError> {
        let observation = self
            ...
            .ok_or_else(|| ObservationError::UnknownObservationLocation {
                location: procedure_location(&coordinate, key_position),
            })?;
        Ok(IndexExclusionConstraintOperatorProcedureSourceReceipt {
...
fn compute_procedure_digest(
    predecessor_digest: &str,
    observations: &[IndexExclusionConstraintOperatorProcedureObservation],
) -> String {
    let mut hasher = Sha256::new();
    hasher.update(INDEX_EXCLUSION_CONSTRAINT_OPERATOR_PROCEDURE_DIGEST_DOMAIN_V1);
...
fn procedure_location(
    coordinate: &IndexExclusionConstraintCoordinate,
    key_position: u32,
) -> String {
    format!(
        "{}/exclusion-operators/{key_position}/procedure",
