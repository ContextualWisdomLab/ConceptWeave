//! PostgreSQL ordinary exclusion-constraint transform-converter strictness evidence.
//!
//! Converter definition, owner, object-level `EXECUTE` ACL, function-local configuration,
//! invoker/definer context, and leakproof classification do not determine `pg_proc.proisstrict`.
//! PostgreSQL exposes strictness as independent function state and `CREATE FUNCTION` permits it to be
//! changed independently of the converter coordinate. This successor therefore binds raw same-row
//! `proisstrict` for every exact converter direction without inventing a strict-only ordinary-EXCLUDE
//! admission rule.
...
const INDEX_EXCLUSION_CONSTRAINT_OPERATOR_PROCEDURE_TRANSFORM_CONVERTER_STRICTNESS_DIGEST_DOMAIN_V1: &[u8] =
    b"conceptweave.postgres_schema_snapshot.v3.relation_partition.index_partition.exclusion_constraint.operator.procedure.transform_converter.strictness.v1";
const SHA256_DIGEST_PREFIX: &str = "sha256:";

/// Raw same-row `pg_proc.proisstrict` evidence for one nonzero transform converter direction.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessObservation {
...
impl IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessObservation {
    /// Records raw converter `pg_proc.proisstrict` while repeating exact predecessor binding identity.
    pub fn new(
        coordinate: IndexExclusionConstraintCoordinate,
        key_position: u32,
        transform_type: QualifiedTypeName,
    ...
    /// Returns raw `pg_proc.proisstrict` from the exact converter function row.
    #[must_use]
    pub const fn is_strict(&self) -> bool {
        self.is_strict
    }

    /// Returns the collision-safe evidence location for this converter strictness fact.
    #[must_use]
    pub fn canonical_location(&self) -> String {
        procedure_transform_converter_strictness_location(
            &self.coordinate,
            self.key_position,
            &self.transform_type,
            self.direction,
...
/// Immutable provenance receipt for one exact converter strictness observation.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessSourceReceipt {
    source_id: String,
    connection_policy_binding: String,
    source_digest: String,
    extractor_revision: String,
    observed_at_utc: String,
    location: IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessObservation,
}

impl IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessSourceReceipt {
    ...
    /// Returns the owner-computed converter strictness successor digest.
    #[must_use]
    ...
    /// Returns the exact validated converter strictness observation.
    #[must_use]
...
/// Complete raw converter `pg_proc.proisstrict` evidence over one exact leakproof predecessor.
///
/// This layer is observational. Both strict and non-strict states are representable, but they
/// produce distinct successor digests and must cover the predecessor converter-direction inventory
/// exactly.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessSnapshot {
...
impl IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessSnapshot {
    /// Creates complete strictness evidence for every converter direction in the predecessor.
    pub fn new(
        leakproof_snapshot: &IndexExclusionConstraintOperatorProcedureTransformConverterLeakproofSnapshot,
        mut observations: Vec<
            IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessObservation,
        >,
    ) -> Result<Self, ObservationError> {
        observations.sort_by_key(strictness_key);

        let expected = leakproof_snapshot
            .observations()
            .iter()
            .map(|observation| {
                strictness_coordinate_key(
        ...
        let snapshot_digest = compute_transform_converter_strictness_digest(
        ...
        Ok(Self {
            ...
            snapshot_digest,
            converter_snapshot_digest: leakproof_snapshot.converter_snapshot_digest().to_owned(),
    ...
    /// Returns the domain-separated converter strictness successor digest.
    #[must_use]
    ...
    /// Issues exact provenance for one observed converter strictness identity.
    ...
    > {
        let observation = self
            ...
            .ok_or_else(|| ObservationError::UnknownObservationLocation {
                location: procedure_transform_converter_strictness_location(
                    &coordinate,
                    key_position,
                    &transform_type,
                    direction,
...
struct ConverterStrictnessCoordinateKey {
    schema_name: String,
...
fn compute_transform_converter_strictness_digest(
    predecessor_digest: &str,
    observations: &[IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessObservation],
) -> String {
    let mut hasher = Sha256::new();
    hasher.update(
        INDEX_EXCLUSION_CONSTRAINT_OPERATOR_PROCEDURE_TRANSFORM_CONVERTER_STRICTNESS_DIGEST_DOMAIN_V1,
    );
...
fn strictness_key(
    observation: &IndexExclusionConstraintOperatorProcedureTransformConverterStrictnessObservation,
) -> ConverterStrictnessCoordinateKey {
    strictness_coordinate_key(
        observation.coordinate(),
        observation.key_position(),
        observation.transform_type(),
...
fn procedure_transform_converter_strictness_location(
    coordinate: &IndexExclusionConstraintCoordinate,
    key_position: u32,
    transform_type: &QualifiedTypeName,
    direction: IndexExclusionConstraintOperatorProcedureTransformConverterDirection,
    ...
    format!(
        "{}/exclusion-operators/{key_position}/procedure-transform-converters/{transform_schema}.{transform_name}/{}/strictness",
