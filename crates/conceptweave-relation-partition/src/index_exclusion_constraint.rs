...
//! evidence with `conparentid`, `conislocal`, and `coninhcount`. Index exclusion operators alone are
//! therefore insufficient source identity. This successor retains the exact exclusion-constraint
//! catalog edge without changing any issued predecessor digest domain.
...
/// Exact `pg_constraint` partition state for one index-backed `EXCLUDE` constraint.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl IndexExclusionConstraintObservation {
    /// Records an exclusion constraint with `conparentid = 0`, `conislocal = true`, and
    /// `coninhcount = 0`.
    pub fn root(coordinate: IndexExclusionConstraintCoordinate) -> Result<Self, ObservationError> {
        Self::new(coordinate, None, true, 0)
    }

    /// Records one exact partitioned exclusion-constraint state.
    pub fn partition(
        coordinate: IndexExclusionConstraintCoordinate,
...
/// Immutable provenance receipt for one exclusion-constraint partition-state observation.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl IndexExclusionConstraintSourceReceipt {
    ...
    /// Returns the exact exclusion-constraint partition-state observation.
    #[must_use]
...
/// Complete PostgreSQL `EXCLUDE` constraint partition state over one exact index-partition snapshot.
///
/// PostgreSQL 18 marks the backing index of an exclusion constraint with `indisexclusion`. Temporal
/// PRIMARY KEY/UNIQUE constraints can also carry that flag, so those same-name key constraints are
/// intentionally excluded from this family and remain owned by the key-constraint parentage
/// successors. Every remaining exclusion index must have one explicit exclusion-constraint catalog
/// observation. Attached exclusion constraints must identify the exact parent constraint and carry
/// PostgreSQL's `(conislocal=false, coninhcount=1)` state; roots remain `(true, 0)`.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl IndexExclusionConstraintSnapshot {
    /// Creates complete exclusion-constraint partition evidence over an exact predecessor chain.
    pub fn new(
        ...
        let expected = expected_exclusion_constraints(base_snapshot)?;
...
fn expected_exclusion_constraints(
    base_snapshot: &PostgresSchemaSnapshotV3,
) -> Result<BTreeSet<IndexExclusionConstraintCoordinate>, ObservationError> {
    ...
    for relation in base_snapshot.relations() {
        for index in relation.indexes() {
            ...
            if flags.exclusion()
                && !relation.constraints().iter().any(|constraint| {
                    is_key_constraint(constraint) && constraint.constraint_name() == index.index_name()
                })
            {
                expected.insert(IndexExclusionConstraintCoordinate::new(
...
fn expected_parent_constraint(
    ...
    let index_coordinate = IndexPartitionCoordinate::new(
    ...
    let membership = index_partition_snapshot
        .observations()
        .iter()
        .find(|candidate| candidate.coordinate() == &index_coordinate)
    ...
    let parent_relation = base_snapshot
        ...
        .ok_or_else(|| invalid("index_exclusion_constraint_parent_coordinate"))?;
    let parent_definition = parent_relation
        .indexes()
        .iter()
        .find(|index| index.index_name() == parent_index.index_name())
        .ok_or_else(|| invalid("index_exclusion_constraint_parent_coordinate"))?;
    let parent_flags = parent_definition
        .catalog_flags()
        .ok_or_else(|| invalid("index_exclusion_constraint_catalog_flags"))?;
    let parent_is_key = parent_relation.constraints().iter().any(|constraint| {
        is_key_constraint(constraint) && constraint.constraint_name() == parent_index.index_name()
    });
    if !parent_flags.exclusion() || parent_is_key {
        return Err(invalid("index_exclusion_constraint_parent_index"));
    }

    Ok(Some(IndexExclusionConstraintCoordinate::new(
