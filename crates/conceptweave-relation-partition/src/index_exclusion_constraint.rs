...
impl Ord for IndexExclusionConstraintCoordinate {
    fn cmp(&self, other: &Self) -> Ordering {
        (
            self.schema_name.as_str(), self.relation_name.as_str(),
            self.relation_kind.token(), self.constraint_name.as_str(),
        ).cmp(&(
            other.schema_name.as_str(), other.relation_name.as_str(),
            other.relation_kind.token(), other.constraint_name.as_str(),
        ))
    }
}
...
impl IndexExclusionConstraintCoordinate {
    ...
    pub fn new(
        schema_name: impl Into<String>, relation_name: impl Into<String>,
        relation_kind: RelationKind, constraint_name: impl Into<String>,
        ...
        if !matches!(relation_kind, RelationKind::Table | RelationKind::PartitionedTable) {
            return Err(invalid("index_exclusion_constraint_relation_kind"));
        }
        Ok(Self { schema_name, relation_name, relation_kind, constraint_name })
    }
    ...
    #[must_use] pub fn relation_name(&self) -> &str { &self.relation_name }
    /// Returns the exact owning relation kind.
    #[must_use] pub const fn relation_kind(&self) -> RelationKind { self.relation_kind }
    ...
    pub fn canonical_location(&self) -> String {
        format!(
            ...
            escape_json_pointer_token(&self.schema_name), self.relation_kind.token(),
            escape_json_pointer_token(&self.relation_name), escape_json_pointer_token(&self.constraint_name)
        )
    }
}
...
impl IndexExclusionConstraintObservation {
    ...
    pub fn partition(
        ...
        Self::new(coordinate, backing_index, Some(parent_constraint), is_local, inheritance_count)
    }
    ...
    fn new(
        ...
        if parent_constraint.as_ref().is_some_and(|parent| parent == &coordinate) {
            return Err(invalid("index_exclusion_constraint_parent"));
        }
        Ok(Self { coordinate, backing_index, parent_constraint, is_local, inheritance_count })
    }
    ...
    #[must_use] pub const fn backing_index(&self) -> &IndexPartitionCoordinate { &self.backing_index }
    /// Returns the exact resolved parent constraint, or `None` for `conparentid = 0`.
    #[must_use] pub const fn parent_constraint(&self) -> Option<&IndexExclusionConstraintCoordinate> { self.parent_constraint.as_ref() }
    /// Returns observed `pg_constraint.conislocal`.
    #[must_use] pub const fn is_local(&self) -> bool { self.is_local }
    /// Returns observed `pg_constraint.coninhcount`.
    #[must_use] pub const fn inheritance_count(&self) -> i16 { self.inheritance_count }
}
...
impl IndexExclusionConstraintSourceReceipt {
    ...
    #[must_use] pub const fn location(&self) -> &IndexExclusionConstraintObservation { &self.location }
}
...
impl IndexExclusionConstraintSnapshot {
    ...
    pub fn new(
        ...
        let rebound = IndexPartitionSnapshot::new(
            base_snapshot, relation_partition_snapshot, index_partition_snapshot.observations().to_vec(),
        )?;
        ...
        let coordinate_set = observations.iter().map(|o| o.coordinate().clone()).collect::<BTreeSet<_>>();
        ...
        let observed_backing = observations.iter().map(|o| o.backing_index().clone()).collect::<BTreeSet<_>>();
        ...
        for observation in &observations {
            let expected_parent = expected_parent_constraint(index_partition_snapshot, &observations, observation)?;
            if observation.parent_constraint() != expected_parent.as_ref() {
                return Err(invalid("index_exclusion_constraint_parentage"));
            }
            let expected_state = if expected_parent.is_some() { (false, 1) } else { (true, 0) };
        ...
        let snapshot_digest = compute_digest(index_partition_snapshot.snapshot_digest(), &observations);
        Ok(Self {
    ...
    #[must_use] pub fn observations(&self) -> &[IndexExclusionConstraintObservation] { &self.observations }

    /// Issues exact provenance for one observed exclusion constraint.
    pub fn source_receipt(
        &self, coordinate: IndexExclusionConstraintCoordinate,
    ) -> Result<IndexExclusionConstraintSourceReceipt, ObservationError> {
        let observation = self.observations.iter().find(|o| o.coordinate() == &coordinate)
            .ok_or_else(|| ObservationError::UnknownObservationLocation { location: coordinate.canonical_location() })?;
...
fn reject_relation_constraint_name_collisions(
    ...
    for observation in observations {
        ...
        let relation = base_snapshot.relations().iter()
            .find(|relation| {
...
fn expected_exclusion_backing_indexes(
    ...
    for relation in base_snapshot.relations() {
        for index in relation.indexes() {
            let flags = index.catalog_flags().ok_or_else(|| invalid("index_exclusion_constraint_catalog_flags"))?;
            ...
            if flags.exclusion() && !temporal_key_backing {
                expected.insert(IndexPartitionCoordinate::new(
                    relation.schema_name(), relation.relation_name(), relation.kind(), index.index_name(),
                )?);
            }
        }
    }
...
fn expected_parent_constraint(
    ...
    let membership = index_partition_snapshot.observations().iter()
        .find(|candidate| candidate.coordinate() == observation.backing_index())
        .ok_or_else(|| invalid("index_exclusion_constraint_backing_index"))?;
    let Some(parent_index) = membership.parent_index() else { return Ok(None); };
    let parent = observations.iter().find(|candidate| candidate.backing_index() == parent_index)
...
fn is_key_constraint(constraint: &TableConstraintObservation) -> bool {
    matches!(constraint, TableConstraintObservation::PrimaryKey(_) | TableConstraintObservation::Unique(_))
}

fn compute_digest(predecessor_digest: &str, observations: &[IndexExclusionConstraintObservation]) -> String {
    ...
    for observation in observations {
        ...
        match observation.parent_constraint() {
            ...
            Some(parent) => { hasher.update([1]); encode_constraint_coordinate(&mut hasher, parent); }
        }
...
fn encode_constraint_coordinate(hasher: &mut Sha256, coordinate: &IndexExclusionConstraintCoordinate) {
    encode_str(hasher, coordinate.schema_name()); encode_str(hasher, coordinate.relation_name());
    encode_str(hasher, coordinate.relation_kind().token()); encode_str(hasher, coordinate.constraint_name());
}
fn encode_index_coordinate(hasher: &mut Sha256, coordinate: &IndexPartitionCoordinate) {
    encode_str(hasher, coordinate.schema_name()); encode_str(hasher, coordinate.relation_name());
    encode_str(hasher, coordinate.relation_kind().token()); encode_str(hasher, coordinate.index_name());
}
fn encode_len(hasher: &mut Sha256, value: usize) {
    let value = u64::try_from(value).expect("Rust target usize must fit into canonical u64 length");
    hasher.update(value.to_be_bytes());
}
fn encode_str(hasher: &mut Sha256, value: &str) { encode_len(hasher, value.len()); hasher.update(value.as_bytes()); }
