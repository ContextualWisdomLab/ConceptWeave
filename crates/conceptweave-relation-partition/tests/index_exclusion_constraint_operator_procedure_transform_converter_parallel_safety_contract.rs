include!(
    "index_exclusion_constraint_operator_procedure_transform_converter_volatility_contract.rs"
);
...
fn ordinary_exclude_transform_converter_parallel_safety_distinguishes_catalog_states() {
    ...
    assert_ne!(
        restricted.snapshot_digest(),
        unsafe_snapshot.snapshot_digest()
    );
}
...
fn ordinary_exclude_transform_converter_parallel_safety_rejects_missing_direction() {
    ...
    let error =
        ...
        .expect_err(
            "every volatility predecessor converter direction must carry proparallel evidence",
        );
    assert_field(
        error,
