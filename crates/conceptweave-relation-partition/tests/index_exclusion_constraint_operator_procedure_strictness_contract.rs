...
fn scalar_snapshot() -> IndexExclusionConstraintOperatorProcedureScalarSnapshot {
    ...
    let base = PostgresSchemaSnapshotV3::new(
        &authorized_source(),
        "extractor-index-exclusion-procedure-strictness-v1",
        "2026-09-17T06:41:47+09:00",
        vec![relation],
        vec![],
        vec![],
    )
...
#[test]
fn ordinary_exclude_preserves_raw_procedure_strictness_and_provenance() {
...
#[test]
fn ordinary_exclude_preserves_non_strict_state_without_inventing_a_postgres_gate() {
    let scalar = scalar_snapshot();
    ...
    assert!(!non_strict.observations()[0].is_strict());
    assert_ne!(strict.snapshot_digest(), non_strict.snapshot_digest());
}

#[test]
fn ordinary_exclude_rejects_procedure_strictness_binding_drift() {
    let scalar = scalar_snapshot();
...
#[test]
fn ordinary_exclude_rejects_operator_strictness_binding_drift() {
    let scalar = scalar_snapshot();
    let error = IndexExclusionConstraintOperatorProcedureStrictnessSnapshot::new(
        &scalar,
        vec![strictness_observation(operator("<>"), procedure("int4eq"), true)],
...
#[test]
fn ordinary_exclude_rejects_missing_procedure_strictness_evidence() {
    let scalar = scalar_snapshot();
    let error = IndexExclusionConstraintOperatorProcedureStrictnessSnapshot::new(&scalar, vec![])
        .expect_err("every governed ordinary EXCLUDE procedure needs raw proisstrict evidence");
    assert_field(
        error,
...
#[test]
fn ordinary_exclude_rejects_duplicate_procedure_strictness_coordinate() {
    let scalar = scalar_snapshot();
    let observation = strictness_observation(operator("="), procedure("int4eq"), true);
    let error = IndexExclusionConstraintOperatorProcedureStrictnessSnapshot::new(
        &scalar,
...
#[test]
fn procedure_strictness_observation_rejects_zero_position() {
    let error = IndexExclusionConstraintOperatorProcedureStrictnessObservation::new(
        coordinate(),
        0,
        operator("="),
        procedure("int4eq"),
...
#[test]
fn procedure_strictness_receipt_rejects_unknown_position() {
    let scalar = scalar_snapshot();
    let snapshot = IndexExclusionConstraintOperatorProcedureStrictnessSnapshot::new(
        &scalar,
        vec![strictness_observation(operator("="), procedure("int4eq"), true)],
