...
fn ordinary_exclude_transform_extension_membership_descends_from_converter_initial_privileges() {
    ...
    let absent =
        ...
        .unwrap();
    let standalone =
        IndexExclusionConstraintOperatorProcedureTransformExtensionMembershipSnapshot::new(
            &absent,
            &transforms,
    ...
    assert_ne!(
        standalone.snapshot_digest(),
        recovery_distinct.snapshot_digest()
    );
}
...
fn ordinary_exclude_transform_extension_membership_is_one_fact_per_transform_row_not_converter_direction()
 {
    ...
    assert_eq!(
        snapshot.observations()[0].transform_type(),
        &custom_payload_type()
    );
    assert_eq!(
        snapshot.observations()[0].target_language_name(),
        "internal"
    );
}
...
fn ordinary_exclude_transform_extension_membership_rejects_completeness_binding_and_duplicates() {
    ...
    let missing =
        ...
        .expect_err(
            "every same-generation pg_transform row needs explicit extension-membership evidence",
        );
    assert_field(
        missing,
    ...
    let observation =
        transform_extension_membership_observation(custom_payload_type(), "internal", None);
    let duplicate =
        IndexExclusionConstraintOperatorProcedureTransformExtensionMembershipSnapshot::new(
            &function_lifecycle,
            &transforms,
