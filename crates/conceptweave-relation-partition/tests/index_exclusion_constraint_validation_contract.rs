...
fn base_snapshot() -> PostgresSchemaSnapshotV3 {
    PostgresSchemaSnapshotV3::new(
        &authorized_source(),
        "extractor-index-exclusion-constraint-validation-v1",
...
fn enforcement_snapshot() -> IndexExclusionConstraintEnforcementSnapshot {
    ...
    .unwrap();
    let timing = IndexExclusionConstraintTimingSnapshot::new(
        &constraints,
        vec![
    ...
    .unwrap();
    IndexExclusionConstraintEnforcementSnapshot::new(
        &timing,
        vec![
...
#[test]
fn exclusion_constraint_cannot_be_observed_as_not_validated() {
    let error = IndexExclusionConstraintValidationObservation::new(parent_constraint(), false)
        .expect_err("PostgreSQL 18 index-backed EXCLUDE constraints are created validated");

    assert_eq!(
        error,
        ObservationError::InvalidObservationField {
            field: "index_exclusion_constraint_validation_state",
        }
    );
}

#[test]
fn exclusion_constraint_validation_inventory_must_be_complete() {
    let enforcement = enforcement_snapshot();
    let error = IndexExclusionConstraintValidationSnapshot::new(
        &enforcement,
        vec![
            IndexExclusionConstraintValidationObservation::new(parent_constraint(), true).unwrap(),
        ],
    )
    .expect_err("every EXCLUDE constraint must retain pg_constraint.convalidated");

    assert_eq!(
        error,
        ObservationError::InvalidObservationField {
            field: "index_exclusion_constraint_validation_completeness",
        }
    );
}

#[test]
fn validated_exclusion_constraint_issues_exact_provenance() {
    let enforcement = enforcement_snapshot();
    let snapshot = IndexExclusionConstraintValidationSnapshot::new(
        &enforcement,
        vec![
            IndexExclusionConstraintValidationObservation::new(parent_constraint(), true).unwrap(),
            IndexExclusionConstraintValidationObservation::new(child_constraint(), true).unwrap(),
        ],
    )
    .expect("PostgreSQL 18 index-backed EXCLUDE constraints are validated evidence");
