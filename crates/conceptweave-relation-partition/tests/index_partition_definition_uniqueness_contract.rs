...
fn index(name: &str, unique: bool) -> IndexObservation {
...
#[test]
fn attached_child_must_preserve_parent_uniqueness_definition() {
    ...
    .expect_err("PostgreSQL cannot attach a non-unique index under a unique parent index");

    assert_eq!(
        error,
        ObservationError::InvalidObservationField {
            field: "index_partition_definition_uniqueness",
        }
    );
}
