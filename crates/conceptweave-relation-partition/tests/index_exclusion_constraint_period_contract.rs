...
fn base_snapshot() -> PostgresSchemaSnapshotV3 {
    PostgresSchemaSnapshotV3::new(
        &authorized_source(),
        "extractor-index-exclusion-constraint-period-v1",
        "2026-09-16T05:50:00Z",
        vec![
            relation(
                "bookings",
...
#[test]
fn ordinary_exclude_requires_conperiod_false() {
    let constraints = exclusion_constraint_snapshot();
    let snapshot = IndexExclusionConstraintPeriodSnapshot::new(
        &constraints,
        vec![
            IndexExclusionConstraintPeriodObservation::new(parent_constraint(), false).unwrap(),
            IndexExclusionConstraintPeriodObservation::new(child_constraint(), false).unwrap(),
        ],
    )
...
#[test]
fn ordinary_exclude_rejects_temporal_conperiod_true() {
    let error = IndexExclusionConstraintPeriodObservation::new(parent_constraint(), true)
        .expect_err("WITHOUT OVERLAPS/PERIOD semantics belong to p/u/f, not ordinary contype=x");

    assert_eq!(
        error,
        ObservationError::InvalidObservationField {
            field: "index_exclusion_constraint_period_state",
        }
    );
}

#[test]
fn exclusion_constraint_period_inventory_must_be_complete() {
    let constraints = exclusion_constraint_snapshot();
    ...
    assert_eq!(
        error,
        ObservationError::InvalidObservationField {
            field: "index_exclusion_constraint_period_completeness",
        }
    );
}

#[test]
fn exact_period_state_issues_provenance() {
    let constraints = exclusion_constraint_snapshot();
    let snapshot = IndexExclusionConstraintPeriodSnapshot::new(
        &constraints,
        vec![
    ...
    .expect("explicit conperiod=false must remain governed evidence");
