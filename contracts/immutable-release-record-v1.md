...
Readback requires a `SemanticRelease` admitted by a client configured with an independently distributed trusted manifest pin. A signed pin can be verified with an independently installed publisher key as specified in `signed-release-manifest-v1.md`. The final directory entry and opened record must be regular files, and readback is capped at the maximum record size; changed or oversized records fail closed. The store compares every header byte with that release and verifies the artifact digest before returning artifact bytes. The record's own manifest digest never grants client trust. This local implementation does not provide cross-host replication, protected key/pin distribution, steward authentication, or a production deployment/backup policy.
