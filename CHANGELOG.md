...
- Source Observation authorization consumes the same monotonic operation budget as adapter execution; source lookup, binding, schema and resource policy work consume that budget, and adapters receive only the remaining duration rather than a reset timeout.
