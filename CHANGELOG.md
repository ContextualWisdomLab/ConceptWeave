...
- Bound every present-material recovery verdict to the exact immutable `IndexExclusionConstraintOperatorProcedureTransformConverterInitialPrivilegeMaterial::digest()` it validated; row absence carries no material digest.
...
- Added and retained focused recovery-validation contracts through the public crate surface rather than inferring governance from observation diagnostics.
- Updated `docs/doctoring/postgresql-index-exclusion-constraint-operator-procedure-transform-converter-initial-privilege-recovery-validation.md` with the verdict-forgery/replay finding, structural RED, causal repair, and evidence-binding invariants.
...
- Recovery readiness is now derived only from private validation evidence produced by the canonical validator; callers cannot manufacture a public `Ready` variant.
- A recovery verdict for present material carries `Some(material.digest())`, so review/publication can verify that the verdict belongs to the immutable observation under consideration. Legitimate row absence carries `None`.
...
- Validation-evidence finding review: `5254963033` at exact pre-finding head `7769b8b21b2d321f6d1f15390643a1f2c512a7cf`.
- Validation-evidence structural RED: `eeb1692ed8e537b761b7acefbdc825ec242d4b38`; the public contract required source-material digest binding before production exposed it and stopped constructing public verdict variants.
- Validation-evidence production repair: `5206290cc8b7dd7128ef849d1acb315a2334e7fc`; present-material verdicts are digest-bound and recovery evidence has no public constructor.
...
- The focused validation contract additionally requires row absence to expose no material digest, every present resolved or damaged material to expose its exact immutable digest, unresolved dimensions to block readiness, and public diagnostics to omit raw OID values.
