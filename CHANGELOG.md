...
- Updated initial-privilege traceability with PostgreSQL 18's ACL-array ordering and multiplicity boundary: grant order is recovery-significant, while PostgreSQL's ACL validator accepts one-dimensional non-null ACL arrays without imposing uniqueness.
...
- Distinct dangling role identities remain distinguishable even when their aggregate unresolved counts are equal. Repeated dangling role identities canonicalize to sorted unique remediation sets while their ACL entries retain source sequence and multiplicity.
...
- ACL-multiplicity finding review: `5256059436` at exact pre-finding head `50fff94eb567fd9b1c2150d4570ca1422acc9a60`; corrected structural RED `9747763c4b2370185bd18039b00e8ceaa9df206c`; production causal repair `55f828f6f6065fac89ae63f521d72d807f875c5b`. The earlier test-only `3ece5938...` contained a type-reference typo and is superseded by `9747763c...`; neither is claimed as an executed failing run.
- The focused contracts now require exact receipt-field retention, source/location-bound row absence, exact present-material digest binding, exact source ACL order and multiplicity, exact dangling-role identity sets, equal-count/different-identity separation, remediation-set sorting/deduplication, readable ACL semantics, damaged-state blocking, and raw-OID redaction from grant/material/validation routine `Debug` surfaces.
