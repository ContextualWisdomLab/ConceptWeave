...
- `ColumnIdentityObservation` now applies the same PostgreSQL identifier rule to schema/relation/column coordinates: exact whitespace is retained, empty and code-zero values fail closed.
- The wider Source Observation representation is still RED because shared `model::validate_nonblank()` remains trim-based and is consumed by other identifier-bearing legacy/v3 constructors. The required repair is a distinct PostgreSQL-identifier validator, not a global weakening of non-identifier text validation.
