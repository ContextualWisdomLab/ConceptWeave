...
- Changed recovery validation to consume the owner-issued `IndexExclusionConstraintOperatorProcedureTransformConverterInitialPrivilegeSourceReceipt` instead of detached ACL material.
- Retained complete non-secret receipt provenance in every verdict: source registry identity, connection-policy binding, source-content digest, extractor revision, observation timestamp, and canonical converter location. Present rows additionally carry the immutable material digest.
- Added `matches_source_receipt()` so review/publication can deterministically reject a verdict replayed against another receipt without exposing dangling role OIDs.
- Kept row absence material-free (`material_digest() == None`) without leaving successful absence validation unbound; the full receipt provenance identifies the exact absent observation that was validated.
- Added focused replay-resistance contracts proving byte-identical ACL material remains distinct validation evidence across converter locations and distinct content snapshots.
- Updated `docs/doctoring/postgresql-index-exclusion-constraint-operator-procedure-transform-converter-initial-privilege-recovery-validation.md` with both receipt-binding findings, REDs, causal repairs, and provenance invariants.
...
- Validation no longer treats source-content digest plus location as the whole receipt. Equal content can remain content-identical while source registry, policy, extractor revision, or observation epoch differs; those provenance fields are retained separately.
- Present recovery evidence carries `Some(material.digest())`; row absence carries `None`. Both states retain the complete owner-issued receipt binding.
...
- Detached-material replay finding review: `5255023556` at exact pre-finding head `83b23116bad4106c2fe1c71953fb2453175dad0c`; structural RED `8734883edcb145933030cccd7177cff95abde772`; first receipt-input repair `9c7e7e8a45eb1a75e43e6a5fe35c5e6f8dec6de2`.
- Complete-receipt finding review: `5255032673` at exact head `d16254bbf116c955a14ca3d6d92b48b70cfb6fa6`; structural RED `c5f9686ec16c27311abeefd858b0d317ac3c4d6d`; production repair `b466a6fcb207061684e104a51010fed6a0bbca55`.
...
- The focused validation contract requires exact receipt-field retention, source/location-bound row absence, exact present-material digest binding, `matches_source_receipt()` rejection across other receipts, unresolved dimensions to block readiness, and public diagnostics to omit raw OID values.
