...
- The wider Source Observation representation remains RED because `model.rs` and `representation_v3.rs` still contain identifier-bearing call sites on trim-based generic validation. The required remaining repair is a distinct PostgreSQL-identifier path, not a global weakening of non-identifier text validation.
