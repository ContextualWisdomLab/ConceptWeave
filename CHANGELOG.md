...
The immediately preceding Source Observation/recovery-validation decision surface is preserved at `docs/archive/CHANGELOG-through-4967f12c.md`; its matching product/technical surface is `docs/archive/product-technical-gap-baseline-through-4967f12c.md`. Earlier recovery surfaces remain under the `through-ae860199`, `through-c1599559`, `through-440d5e3f`, `through-8348be96`, and `through-5daf2a57` archives. Focused rationale and primary-source traceability remain under `docs/doctoring/`.
...
- The initial-privilege material retains the complete source-order grant vector after digest construction. `grants()` exposes the observed ACL sequence for semantic/recovery inspection rather than a normalized sequence.
- Added log-safe grant semantics accessors for PUBLIC shape, resolved grantee role, unresolved-grantee presence, resolved grantor role, unresolved-grantor presence, and grant option. Raw dangling OIDs remain unavailable from the ordinary grant/material surface and stay purpose-bound to receipt-backed recovery validation.
...
- Retained canonical sorted/deduplicated unresolved grantee/grantor OID sets inside initial-privilege material so recovery evidence preserves the exact missing PostgreSQL role identities rather than only aggregate counts. These remediation sets do not reorder the source ACL itself.
...
- Updated initial-privilege traceability with PostgreSQL 18's ACL-array ordering requirement: grant order is recovery-significant because dump/restore may need grant-option providers before dependent grants.
...
- `IndexExclusionConstraintOperatorProcedureTransformConverterInitialPrivilegeMaterial` no longer reduces the complete ACL to an opaque digest plus counts. It retains the exact source-order grant sequence so `observe -> validate -> review` can inspect PUBLIC/resolved-role/grant-option/unresolved-kind semantics without falsifying PostgreSQL ACL ordering. Raw dangling OID disclosure remains restricted to recovery validation.
- Removed the material constructor's `grants.sort()` normalization. PostgreSQL 18 explicitly treats ACL-array order as significant for client utilities such as `pg_dump`, because grants with grant option may need to precede dependent grants. Two source ACL arrays with the same entries in different order therefore remain distinct material/source identities.
- Duplicate ACL-entry rejection is now order-independent via a separate set check; validation no longer relies on sorting entries adjacent before testing duplication.
- `IndexExclusionConstraintOperatorProcedureTransformConverterInitialPrivilegeMaterial` retains exact canonical dangling-role sets without changing resolved-role or unresolved-role entry framing. The Source Observation digest commits the original ACL entry order and identities and does not add derived diagnostic fields.
...
- Distinct dangling role identities remain distinguishable even when their aggregate unresolved counts are equal. Repeated dangling role identities canonicalize to sorted unique remediation sets while their ACL entries retain source sequence.
...
- Complete-ACL readability finding review: `5255771158` at exact pre-finding head `ae86019978a9d1fbce6b905d65df92835e851b39`; structural RED `3d03f9b9d3ce27ae2d96732ec03780a345493afc`; production causal repair `d406375f8989dae5dc06c50c1f4a776408a00bd8`.
- Source-order integrity finding review: `5255912747` at exact pre-finding head `4967f12cb90ac41ec42263bfed47ba6bcbb49735`; structural RED `13e51561cac125f2a942e7ca0d0dd39433040426`; production causal repair `966e2136c15aa049f72acfebf2c1a478d09c66ef`. The RED is a source-level contract and is not claimed as an executed failing run.
- The focused contracts now require exact receipt-field retention, source/location-bound row absence, exact present-material digest binding, exact source ACL order, exact dangling-role identity sets, equal-count/different-identity separation, remediation-set sorting/deduplication, readable ACL semantics, damaged-state blocking, and raw-OID redaction from grant/material/validation routine `Debug` surfaces.
